...
if (isset($_POST['mode']) && $_POST['mode'] == 'addInvoiceNonComm') {

    // var_dump($_POST);

    // exit;
    /*
        $lastSOID = $obj_invoicecommornon->NonComInvoice_lastinserted_id();
    ...
    */
    ...
    $lastSOID = $obj_invoicecommornon->NonComInvoice_last();
    $previousYr = date("y", strtotime("-1 year"));  //last year 
    $nextYr = date('y', strtotime('+1 year'));
    ...
    // $obj_invoicecommornon->setreference_number($_POST['txt_reference_number']);
    ...
    $seid = $obj_invoicecommornon->addNonComInvoice();
    ...
    $obj_estimatecommornon->changenoncommPaidorder($_POST['txt_sc_no'], $statusPaidorder);

    $obj_estimatecommornon->changenoncommPaidorderstatus($_POST['txt_sc_no'], $statusPaidorder, $statusdate);
    ...
    for ($i = 0; $i < count($_POST['txt_itemtypes']); $i++) {
        ...
        $obj_invoicecommornon->settypes_id($_POST['txt_itemtypes'][$i]);
        ...
        $status = $obj_invoicecommornon->addNonComInvoiceProduct(["group_id" => $_POST['group_id'][$i]]);
    }

    $obj_invoicecommornon->updateSOStatus($so_id);

    if ($status) {
        echo json_encode($seid);
    } else {
...
if (isset($_POST['mode']) && $_POST['mode'] == 'updateSalesInvoice') {
    ...
    try {
        $seid = $obj_invoicecommornon->updateNonComInvoice($_POST['txt_so_id']);

        $deleteStatus = $obj_invoicecommornon->deleteNonComInvoiceProduct($_POST['txt_so_id']);
        ...
        for ($i = 0; $i < count($_POST['txt_itemtypes']); $i++) {
            ...
            $obj_invoicecommornon->setpk_id($_POST['txt_so_id']);
            ...
            if (!$status) {
                throw new Exception("Insert failed at index $i");
            }
        }
...
if (isset($_POST['mode']) && $_POST['mode'] == 'getJobOrder') {
    ...
    $status = $obj_invoicecommornon->getNonJobOrder();
...
if (isset($_POST['mode']) && $_POST['mode'] == 'getJobOrderEdit') {
    ...
    // $status = $obj_invoicecommornon->getNonJobOrderEdit();
    $status = $obj_invoicecommornon->getNonJobOrderEditAll();
...
if (isset($_POST['mode']) && $_POST['mode'] == 'listInvoice') {
    ...
    $status = $obj_invoicecommornon->listNonCommInvoice();
}
...
if (isset($_POST['mode']) && $_POST['mode'] == 'getSIEditValues') {
    ...
    $getSEId = $obj_invoicecommornon->getNonCommInvoiceById($_POST['eid']);
    ...
    if (!empty($datas)) {
        ...
        $getSEProducts = $obj_invoicecommornon->getNonCommInvoiceProductByPROId($_POST['eid']);
        ...
        while ($datass = mysqli_fetch_array($getSEProducts)) {
            ...
            $getSEProduct = $obj_invoicecommornon->getNonCommInvoiceProductById($datass['PK_SEP_ID']);
