        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <h3 class="card-title">Quick Example <small>jQuery Validation</small></h3>
                        ...
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form class="form-horizontal theme-form" id="form_sales_invoice_add" autocomplete="off"
...
$('#txt_types').on('change', function() {
    ...
    getItemType();
    if (types == 1) {
        $('.txt_itemtypes').html('<option selected >Select One</option><option value="1">Product</option>');
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            setTimeout(function() {
                ...
                //  $('.txt_product_name').trigger('chosen:updated');
            }, 1000);
        },
        error: function(response) {
            console.log(response);
...
$('#form_sales_invoice_add').validate({
    rules: {
        txt_sc_no: {
            required: true
        },
    ...
    submitHandler: function(form) {
        var formData = new FormData($('#form_sales_invoice_add')[0]);
        $.ajax({
            url: "modules/invoice/ajax_functions.php",
            data: formData,
            type: 'post',
            ...
            success: function(response) {
                if (response == 1) {
                    swal({
                        title: "Success!",
                        text: "Sales Invoice has been added successfully!.",
                        type: "success",
                        timer: 1000,
                        buttons: false,
                    }).then(function() {
