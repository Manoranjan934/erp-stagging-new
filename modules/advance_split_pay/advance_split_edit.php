                            ...
                            <?php } elseif ($_GET['type'] == 2) { ?>
                                ...
                                <div class="card-footer text-right">
                                    ...
                                    <span class="col-md-2">
                                        ...
                                        <?php if ($_GET['type'] == 1) { ?>
                                            <input type="hidden" name="mode" value="editAdvancecomm" />
                                        <?php } elseif ($_GET['type'] == 2) { ?>
                                            <input type="hidden" name="mode" value="editAdvanceNonComm" />
                                        <?php } ?>
...
<script>
    ...
    $("#txt_customer_name").change(async function() {
        var esID;
        //alert($('option:selected', this).val());
        var c_name = $('option:selected', this).val();
        if (c_name != '') {
            await getEstimatesByCustomer(c_name);
        } else {
    ...
    function getEstimatesByCustomer(customerId) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    for (var i = 0; i < response.length; i++) {
                        var pkid = response[i].esid;
                        $('#txt_estimate_name').append($('<option value="' + pkid + '">' + response[i].sono + '</option>'));
                    }
                    $('.txt_estimate_name').trigger('change');
                    console.log(response[i]?.esid);
                    resolve(response)
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (soid == 0) {
                        ...
                        // getEstimateVAlue(response[0].customer_id,response[0].fk_es_id );
            ...
            $.ajax({
                ...
                success: async function(response) {
                    if (soid == 0) {
                        ...
                        await getEstimatesByCustomer(response[0].customer_id);
