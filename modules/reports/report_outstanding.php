...
$query = "SELECT sum(est.grand_total) as totalAmt,cus.cus_name,cus.pk_cus_id,GROUP_CONCAT(est.PK_ES_ID ORDER BY est.PK_ES_ID) as es_id FROM `erp_estimate_comm` as est,erp_customer_master as cus where cus.pk_cus_id=est.customer_id " . $appendQuery . " group by customer_id";
...

{
  ...
  $query = "SELECT ( SELECT SUM(est.grand_total ) grand_total FROM erp_estimate_comm est WHERE FIND_IN_SET(est.PK_ES_ID, bp.fk_es_id) > 0  ) as grand_total 



  FROM ".BULK_PAYMENT_COMM." bp  WHERE   bp.fk_es_id IN(".$esID.") ";
...
$query = "SELECT sum(est.grand_total) as totalAmt,cus.cus_name,cus.pk_cus_id,GROUP_CONCAT(est.PK_ES_ID) as es_id FROM `erp_estimate_noncomm` as est,erp_customer_master as cus where cus.pk_cus_id=est.customer_id " . $appendQuery . " group by customer_id";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($data = mysqli_fetch_array($result)) {
        ...
        $bulkNpay = getNbulkPayment($data['es_id']);
        ...
        $outNTotal =$data['totalAmt'] - ($receipts + $advance) ;
        ...
        $tableBody .= '
                              ...
                              <tr>
								...
								<td style="text-align:right;">' . number_format( $bulkNpay, 2) . '</td>



								<td style="text-align:right;">' . number_format($bulkNamt, 2) . '</td>
...

{
  ...
  $query = "SELECT ( SELECT SUM(est.grand_total ) grand_total FROM erp_estimate_noncomm est WHERE FIND_IN_SET(est.PK_ES_ID, bp.fk_es_id) > 0  ) as grand_total 



  FROM ".BULK_PAYMENT_NONCOMM." bp  WHERE   bp.fk_es_id IN(".$esID.") ";
