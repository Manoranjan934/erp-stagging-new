...
<button type="submit" name="submit" class="btn btn-info">Search</button>
                    ...
                    <form class="row" autocomplete="off" method="post" id="rmcyte_employee">
                        ...
                        <div class="col-lg-2 form-group">
                            <label for="name">Mode / Franchise <span class="text-danger">*</span></label>
                            <select class="form-control txt_franchise_name " name="txt_franchise_name"
                                ...
                                <option value="<?php echo $frans_data[$i]['pk_cat_id']; ?>">
                        ...
                        <div class="card-body">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                <div class="tab-pane fade show active" id="nav-waiting" role="tabpanel"
                                    ...
                                    <?php if (isset($_GET['typ']) && $_GET['typ'] == 1) { ?>
...
$(document).ready(function() {
    ...
    var dataRecords = $('#estimateTable').DataTable({
        ...
        "footerCallback": function(row, data, start, end, display) {
            ...
            var totalAmount = api
                .column(<?php echo ($_GET['typ'] == 1) ? 6 : 10; ?>)
                .data()
                .reduce(function(a, b) {
                    return intVal(a) + intVal(b);
                }, 0);

            $(api.column(<?php echo ($_GET['typ'] == 1) ? 6 : 10; ?>).footer()).html('&emsp;&emsp;&emsp;&emsp;₹' + totalAmount.toFixed(2));
        }
    });
...
function getCity(stateId, cityid) {
    if ($("#txt_state").find(":selected").attr('data-stid') != '' && $("#txt_state").find(":selected").attr('data-stid') != null) {
        var stateId = $("#txt_state").find(":selected").attr('data-stid');
        $('#txt_city').empty();
        ...
        $.post("modules/customer/ajax_functions.php", {
            ...
            setTimeout(function() {
                $("#form_customer_update #txt_city").find('option[value="' + cityid + '"]').attr("selected", true);
                $("#txt_city").change();
            }, 1000);
        });
    } else {
