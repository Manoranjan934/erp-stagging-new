...
if(isset($_POST['mode']) && $_POST['mode']=='addEstimateComm'){
	$lastSOID = $obj_estimatecommornon->EstimateComm_lastinserted_id();
    $sCid = $lastSOID + 1;
...
$obj_estimatecommornon->setsale_date(date('Y-m-d',strtotime($txt_pi_date)));
 ...
 //   $obj_estimatecommornon->setcaltype5($_POST['txt_cal_type5']);
	...
	$soid = $obj_estimatecommornon->updateEstimateComm($_POST['txt_so_id']);
	...
	if(isset($_POST['mode']) && $_POST['mode'] == 'listEstimateCommPaidcomplete') {
	
        $obj_estimatecommornon->listEstimateCommPaidcomplete();
    }
