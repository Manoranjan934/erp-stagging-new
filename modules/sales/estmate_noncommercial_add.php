<?php

// error_reporting(E_ALL);
// ini_set("display_errors", 1);
include "classes/class_category.php";
include "classes/class_uom.php";
$obj_cat = new Category('', '', '', '', '', '');
$obj_uom = new Uom('', '', '', '', '');
$getuom = $obj_uom->getalluom();
$uom_data = array();
while ($uom_rows = mysqli_fetch_array($getuom)) {
    $uom_data[] = $uom_rows;
}

$getcate = $obj_cat->getallcategory();
$cat_data = array();
while ($cat_rows = mysqli_fetch_array($getcate)) {
    $cat_data[] = $cat_rows;
}

include "classes/class_customer.php";
$obj_cus = new Customer('', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', $GLOBALS["___mysqli_ston"]);
$getcustomer = $obj_cus->getAllCustomer();
$cus_data = array();
while ($cus_rows = mysqli_fetch_array($getcustomer)) {
    $cus_data[] = $cus_rows;
}
$getAllFrancise = $obj_cus->getAllFrancise();
$fran_data = array();
while ($fran_rows = mysqli_fetch_array($getAllFrancise)) {
    $fran_data[] = $fran_rows;
}
$getAllStates = $obj_cus->getAllStates();
$state_data = array();
while ($state_rows = mysqli_fetch_array($getAllStates)) {
    $state_data[] = $state_rows;
}


include("classes/class_sale_order.php");
$obj_saleorder = new Sale_order('', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '');
$gettypes = $obj_saleorder->getAllTypes();

// error_reporting(E_ALL);
// ini_set("display_errors", 1);
?>

<style>
    .me-2-custom {
        margin-inline-end: 0.5rem;
    }

    table {
        border-collapse: separate;
        border-spacing: 0;
    }

    /* LEFT & RIGHT borders (all rows in group) */
    tr.row_group td:first-child {
        border-left-color: #b2c6fcf5;
    }

    tr.row_group td:last-child {
        border-right-color: #b2c6fcf5;
    }

    tr.item_type_1 td {
        border-top-color: #b2c6fcf5;
    }

    tr.item_type_5:nth-last-child(1 of .item_type_5) td {
        border-bottom-color: #b2c6fcf5;
    }

    /* tr.row_group:not(.row_group + tr.row_group) td {
        border-top-color: #ff3d00;
    }

    tr.row_group+tr:not(.row_group) td {
        border-top-color: #ff3d00;
    } */
</style>
<div class="content-wrapper" style="min-height: 1342.88px;">
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Add Non Commercial Estimate</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="index.php">Home</a></li>
                        <li class="breadcrumb-item active">Add Non Commercial Estimate</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>

    <!-- Main content -->
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <!-- left column -->
                <div class="col-md-12">
                    <!-- jquery validation -->
                    <div class="card card-primary">
                        <!-- <div class="card-header">
                <h3 class="card-title">Quick Example <small>jQuery Validation</small></h3>
              </div> -->
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form class="form-horizontal theme-form" id="form_estimate_noncomm_add" autocomplete="off"
                            novalidate="novalidate" onsubmit="return false">
                            <div class="card-body">
                                <div id="stepwizard">
                                    <!--<div class="one-half-column">
                                        <div class="form-group">
                                            <label for="" class="control-label">SO No </label>
                                            <div class="control-field">
                                                <input type="text" class="form-control txt_sono" name="txt_sono"
                                                    id="txt_sono" placeholder="Auto" title="SO NO" readonly="">
                                                <span class="error" id="txt_sono_error"></span>
                                            </div>
                                        </div>

                                        <div class="form-group">
                                            <label for="" class="control-label">Date <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <input type="text" class="form-control txt_pi_date hasDatepicker valid"
                                                    name="txt_pi_date" id="txt_pi_date" placeholder="DD/MM/YYYY"
                                                    required="" aria-required="true" aria-invalid="false">
                                                <span class="error" id="txt_pi_date_error"></span>
                                            </div>
                                        </div>

                                        <div class="form-group">
                                            <label for="" class="control-label">Customer <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <div>
                                                    <select class="form-control txt_customer_name chosen_required"
                                                        name="txt_customer_name" id="txt_customer_name" title="Customer"
                                                        required="" aria-required="true">
                                                        <option value="">Select Customer</option>
                                                        <?php
                                                        for ($i = 0; $i < count($cus_data); $i++) {
                                                        ?>
                                                        <option value="<?php echo $cus_data[$i]['pk_cus_id']; ?>">
                                                            <?php echo $cus_data[$i]['cus_name']; ?></option>
                                                        <?php
                                                        }
                                                        ?>
                                                    </select>
                                                </div>
                                                <span class="error" id="txt_customer_name-error"></span>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="control-label">Product <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <div>
                                                    <select class="form-control txt_product_name chosen_required"
                                                        name="txt_product_name" id="txt_product_name" title="Product"
                                                        required="" aria-required="true">
                                                        <option value="">Select Product</option>

                                                    </select>
                                                </div>
                                                <span class="error" id="txt_product_name-error"></span>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="control-label">Category <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <div>
                                                    <select class="form-control txt_category chosen_required"
                                                        name="txt_category" id="txt_category" title="Category"
                                                        required="" aria-required="true">
                                                        <option value="">Select Category</option>

                                                    </select>
                                                </div>
                                                <span class="error" id="txt_customer_name-error"></span>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="control-label">Reference Number </label>
                                            <div class="control-field">
                                                <input type="text" class="form-control txt_reference_number"
                                                    name="txt_reference_number" id="txt_reference_number"
                                                    placeholder="Reference Number" title="Reference Number">
                                                <span class="error" id="txt_reference_number_error"></span>
                                            </div>
                                        </div>
                                    </div>-->

                                    <div class="col-4">
                                        <div class="form-group">
                                            <label for="" class="control-label">Estimate No </label>
                                            <div class="control-field">
                                                <input type="text" class="form-control txt_sono" name="txt_sono"
                                                    id="txt_sono" placeholder="Auto" title="SO NO" readonly="">
                                                <span class="error" id="txt_sono_error"></span>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="control-label">Customer <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <div>
                                                    <select class="form-control txt_customer_name "
                                                        name="txt_customer_name" id="txt_customer_name" onchange="customerChange()">
                                                        <option value="">Select Customer</option>
                                                        <?php
                                                        for ($i = 0; $i < count($cus_data); $i++) {
                                                        ?>
                                                            <option value="<?php echo $cus_data[$i]['pk_cus_id']; ?>">
                                                                <?php echo $cus_data[$i]['cus_name'] . "- (" . $cus_data[$i]['cus_code'] . ")"; ?>
                                                            </option>
                                                        <?php
                                                        }
                                                        ?>
                                                    </select>
                                                </div>
                                                <span class="error" id="txt_customer_name-error"></span>
                                            </div>
                                        </div>

                                        <!--  <div class="form-group">
                                            <label for="" class="control-label">Types <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <div>
                                                    <select class="form-control txt_types chosen_required"
                                                        name="txt_types" id="txt_types" title="Types" required=""
                                                        aria-required="true">
                                                        <option value="">Select Types</option>
                                                        <option value="1">Commercial</option>
                                                        <option value="2">Non Commercial</option>

                                                    </select>
                                                </div>
                                                <span class="error" id="txt_types-error"></span>
                                            </div>
                                        </div>-->

                                        <!--  <div class="form-group">
                                            <label for="" class="control-label">Orientation <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <div>
                                                    <select class="form-control txt_orientation chosen_required"
                                                        name="txt_orientation" id="txt_orientation"
                                                        title="Orientation" required="" aria-required="true">
                                                        <option value="">Select Orientation</option>
                                                        <option value="1">landscape</option>
                                                        <option value="2">portrait</option>

                                                    </select>
                                                </div>
                                                <span class="error" id="txt_orientation-error"></span>
                                            </div>
                                        </div>-->
                                        <?php
                                        date_default_timezone_set("Asia/Calcutta"); //India time (GMT+5:30)
                                        ?>
                                        <div class="form-group">
                                            <label for="" class="control-label">Date <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <input type="text" class="form-control txt_pi_date  " name="txt_pi_date"
                                                    placeholder="DD/MM/YYYY" value="<?php echo date('d-m-Y'); ?>"
                                                    readonly="" />
                                                <span class="error" id="txt_pi_date_error"></span>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="control-label">Estimated Delivery Date <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <input type="text"
                                                    class="form-control txt_delivery_date hasDatepicker valid"
                                                    name="txt_delivery_date" id="txt_delivery_date"
                                                    placeholder="DD/MM/YYYY" aria-invalid="false">
                                                <span class="error" id="txt_delivery_date_error"></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-4 col-md-12">


                                        <div class="form-group">
                                            <label for="" class="control-label">Payment Type </label>
                                            <div class="control-field">
                                                <div>
                                                    <select class="form-control txt_payment_type "
                                                        name="txt_payment_type" id="txt_payment_type">
                                                        <option value="">Select Payment Type</option>
                                                        <option value="1">Cash</option>
                                                        <option value="2">Credit Card</option>
                                                        <option value="3">UPI</option>
                                                        <option value="4">Bank Transfer</option>
                                                        <option value="5">Cheque</option>

                                                    </select>
                                                </div>
                                                <span class="error" id="txt_payment_type-error"></span>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="control-label">Remarks </label>
                                            <div class="control-field">
                                                <textarea class="form-control txt_remarks" name="txt_remarks"
                                                    style="height: 77px !important;" id="txt_remarks"></textarea>
                                                <span class="error" id="txt_remarks_error"></span>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="control-label">State</label>
                                            <div class="control-field">
                                                <select class="form-control txt_state capallfields "
                                                    style=" text-transform:uppercase !important;" id="txt_state"
                                                    name="txt_state">
                                                    <option value="" selected="" data-select2-id="4">SELECT STATE
                                                    </option>
                                                    <?php for ($i = 0; $i < count($state_data); $i++) { ?>
                                                        <option data-code="<?php echo $state_data[$i]['state_code']; ?>"
                                                            value="<?php echo $state_data[$i]['state_code']; ?>">
                                                            <?php echo $state_data[$i]['state_name']; ?></option>
                                                    <?php } ?>
                                                </select>

                                                <span class="error" id="txt_state_error"></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-4 col-md-12">
                                        <div class="form-group">
                                            <label for="" class="control-label">City</label>
                                            <div class="control-field">
                                                <select class="form-control txt_customer_city capallfields "
                                                    style=" text-transform:uppercase !important;" id="txt_customer_city"
                                                    name="txt_customer_city">
                                                    <option value="" selected="" data-select2-id="4">SELECT CITY
                                                    </option>
                                                </select>

                                                <span class="error" id="txt_customer_city_error"></span>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="control-label">Mobile Number <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <input type="text" class="form-control txt_streetarea  "
                                                    name="txt_streetarea" id="txt_streetarea" value="">
                                                <span class="error" id="txt_streetarea_error"></span>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="control-label">Mode / Franchise<span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <select class="form-control txt_franchise " name="txt_franchise"
                                                    id="txt_franchise">
                                                    <option value="">Select Mode / Franchise</option>
                                                    <?php for ($i = 0; $i < count($cat_data); $i++) { ?>
                                                        <option value="<?php echo $cat_data[$i]['pk_cat_id']; ?>">
                                                            <?php echo $cat_data[$i]['cat_name']; ?></option>
                                                    <?php } ?>
                                                </select>

                                                <span class="error" id="txt_customer_city_error"></span>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="control-label">GST Type</label>
                                            <div class="control-field">
                                                <select class="form-control txt_intstate " onchange="cal()"
                                                    name="txt_intstate" id="txt_intstate">
                                                    <option value="1" selected>Inclusive of GST</option>
                                                    <option value="2">Exclusive of GST</option>

                                                </select>
                                                <span class="error" id="txt_customer_city_error"></span>
                                            </div>
                                        </div>
                                        <!-- <div class="form-group">
                                            <label for="" class="control-label">Price Type <span
                                                    class="color">*</span></label>
                                            <div class="control-field">
                                                <div>
                                                    <select class="form-control txt_price_type chosen_required"
                                                        name="txt_price_type" id="txt_price_type"
                                                        title="Price Type" required="" aria-required="true">


                                                    </select>
                                                </div>
                                                <span class="error" id="txt_price_type-error"></span>
                                            </div>
                                        </div>-->

                                    </div>

                                </div>
                            </div>

                            <div class="card-body">
                                <div class="tab-content">
                                    <div class="text-right">


                                        <!--  <span class="btn btn-sm btn-primary btn-mint mar-btm" id="additems"
                                            onclick="addrow();" style="margin-top:-40px"><i class="fa fa-plus mr-2"
                                                aria-hidden="true"></i> Add row
                                        </span>-->
                                    </div>
                                    <div class="table-div table-responsive">
                                        <table class="table-bordered table thead itemTable">
                                            <thead>
                                                <tr>
                                                    <th width="150" valign="middle" colspan="2">Action</th>
                                                    <!-- <th width="5" valign="middle">#</th> -->

                                                    <th width="120">Item Type <span class="color"> *</span></th>
                                                    <th width="120">Item <span class="color"> *</span></th>

                                                    <th width="120">Quantity <span class="color"> *</span></th>
                                                    <th width="120">Price Type <span class="color"> *</span></th>
                                                    <th width="120">Orientation </th>
                                                    <th width="120">Price(₹) <span class="color"> *</span></th>
                                                    <th width="120">Total (₹)<span class="color"> *</span> </th>

                                                </tr>
                                            </thead>
                                            <tbody class="itemclone custom_table_width">
                                                <!--<tr class="nodata">
                                                    <td colspan="10" class="text-center error"> No records available in
                                                        the table !</td>
                                                </tr>-->
                                                <?php
                                                $types_data = array();
                                                $i = 1;
                                                while ($types_rows = mysqli_fetch_array($gettypes)) {

                                                    $isCalendarProduct = (
                                                        isset($types_rows['types_name']) &&
                                                        $types_rows['types_name'] === 'Product (Calendar)'
                                                    );
                                                    $isProductCalendar = ($label === 'Product (Calendar)');
                                                    $innersplattr = '';
                                                    $innersplcls = '';
                                                    $splcls = '';
                                                    if ($types_rows['pk_types_id'] == 4) {
                                                        $innersplattr = 'onclick="addinnersheetrow(1)"';
                                                        $innersplcls = 'addinnercls';
                                                    } else if ($types_rows['pk_types_id'] == 5) {
                                                        $innersplattr = 'onclick="addspecialeffectrow(1)"';
                                                        $innersplcls = 'addsplcls';
                                                    }
                                                    $addinnerdata = '';
                                                    if ($types_rows['pk_types_id'] == 4 || $types_rows['pk_types_id'] == 5) {
                                                        $addinnerdata = '<button type="button" class="btn btn-success btn-number" ' . $innersplattr . ' data-type="plus" data-field="quant[2]"> <span class="fa fa-plus"></span></button>';
                                                    }
                                                    $addspecialdata = '';

                                                    $hideorient = '';
                                                    if ($types_rows['pk_types_id'] == 1) {
                                                        $hideorient = '<option value="">Select Orientation</option><option value="1">Length</option><option value="2">Breadth </option><option value="">NONE </option>';
                                                    } else {
                                                        $hideorient = '<option value="">NONE </option>';
                                                    }
                                                    //$types_data[] = $types_rows;pk_types_id,types_name


                                                    $calendarAttr = $isCalendarProduct ? "data-calendar='1'" : "";

                                                    $prodGroup = in_array($types_rows['pk_types_id'], [1, 2, 3, 4, 5]) ? "prod_group_1 row_group" : "";

                                                    $prodGroupBtn = '';
                                                    if ($types_rows['pk_types_id'] == 1) {
                                                        $prodGroupBtn = <<<EOF
                                                            <span class="btn btn-sm btn-primary me-2-custom" style="cursor:auto">#1</span>
                                                            <button 
                                                                type="button" 
                                                                class="btn btn-success btn-number" 
                                                                data-type="plus" 
                                                                data-field="quant[2]"
                                                                onclick="addProductGroup()"> 
                                                                <span class="fa fa-plus"></span>
                                                            </button>
                                                        EOF;
                                                    }

                                                    echo <<<EOF
                                                    <tr class="{$innersplcls} {$prodGroup} item_type_{$types_rows['pk_types_id']}" data-group_id="1" data-type_id={$types_rows['pk_types_id']}>
                                                        <td width="100">{$prodGroupBtn}</td>
                                                        <td width="50">{$addinnerdata}</td>
                                                        <td>
                                                            <input 
                                                                type="hidden" 
                                                                class="form-control txt_itemtypes txt_itemtypes_{$i} "   
                                                                id="txt_itemtypes_{$i}"
                                                                name="txt_itemtypes[]" 
                                                                value="{$types_rows['pk_types_id']}"
                                                                data-czid="{$i}" 
                                                                data-classids="txt_itemtypes_{$i}" 
                                                            >
                                                            {$types_rows['types_name']}
                                                        </td>
                                                        <td>
                                                            <select class="form-control txt_item txt_item_{$i}"
                                                                id="txt_item_{$i}"
                                                                name="txt_item[]"
                                                                data-czid="{$i}"
                                                                {$calendarAttr}>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input 
                                                                class="form-control txt_product_qty numbersOnly  txt_product_qty_{$i}"
                                                                id="txt_product_qty_{$i}"
                                                                name="txt_product_qty[]" 
                                                                value="0"
                                                                placeholder="Quantity"
                                                                min="0" 
                                                                max="999999" 
                                                                onkeyup="cal()"
                                                            >
                                                        </td>
                                                        <td>
                                                            <select class="form-control txt_price_type txt_price_type_{$i}"
                                                                id="txt_price_type_{$i}"
                                                                name="txt_price_type[]" 
                                                                data-czid="{$i}">
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <select class="form-control txt_orientation txt_orientation_{$i}"
                                                                id="txt_orientation_{$i}"  
                                                                name="txt_orientation[]" 
                                                                onchange="cal()" >
                                                                {$hideorient}
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input 
                                                                type="text" 
                                                                class="form-control pricefield txt_price txt_price_{$i} numberss text-right"
                                                                id="txt_price_{$i}"
                                                                name="txt_price[]" 
                                                                value="0"
                                                                onkeyup="cal()"
                                                            >
                                                            <input 
                                                                type="hidden"
                                                                class="txt_comm txt_comm_{$i}" 
                                                                id="txt_comm"
                                                                name="txt_comm" 
                                                                value=""
                                                            >
                                                            <input 
                                                                type="hidden"
                                                                name="group_id[]" 
                                                                value="1"
                                                            >
                                                        </td>
                                                        <td>
                                                            <input 
                                                                type="text" 
                                                                class="form-control txt_final_total_{$i} numberss txt_final_total text-right"
                                                                id="txt_final_total_{$i}"
                                                                name="txt_final_total[]"
                                                                readonly
                                                            >
                                                        </td>

                                                    </tr>
                                                    EOF;


                                                    // echo '<tr class="' . $innersplcls . '"> 
                                                    //         <td>' . $addinnerdata . '</td>
                                                    //         <td><input type="hidden" class="form-control txt_itemtypes txt_itemtypes_' . $i . ' "   name="txt_itemtypes[]" id="txt_itemtypes_' . $i . '"
                                                    //                 data-czid="' . $i . '" data-classids="txt_itemtypes_' . $i . '" value=' . $types_rows['pk_types_id'] . '>' . $types_rows['types_name'] . '
                                                    //         </td>
                                                    //         <td>
                                                    //             <select class="form-control txt_item txt_item_' . $i . '"
                                                    //                     name="txt_item[]"
                                                    //                     id="txt_item_' . $i . '"
                                                    //                     data-czid="' . $i . '"
                                                    //                     ' . ($isCalendarProduct ? 'data-calendar="1"' : '') . '>
                                                    //             </select>
                                                    //         </td>
                                                    //         <td><input onkeyup="cal()"
                                                    //                 class="form-control txt_product_qty numbersOnly  txt_product_qty_' . $i . '"
                                                    //                 min="0" max="999999" id="txt_product_qty_' . $i . '"
                                                    //                 name="txt_product_qty[]" placeholder="Quantity"
                                                    //                 value="0"></td>
                                                    //         <td><select class="form-control txt_price_type txt_price_type_' . $i . ' "
                                                    //                 data-czid="' . $i . '" name="txt_price_type[]" id="txt_price_type_' . $i . '"
                                                    //                 ></select></td>';

                                                    // echo '<td><select class="form-control txt_orientation txt_orientation_' . $i . '"
                                                    //             name="txt_orientation[]" id="txt_orientation_' . $i . '"  onchange="cal()" >' . $hideorient . ' </select></td>';
                                                    // //  $user_idchksess = ''; if($_SESSION['user_id'] == 1 || $_SESSION['user_id']  == 4  || $_SESSION['user_id']  == 6){  $user_idchksess = '' ;}else{   $user_idchksess = 'readonly';} 

                                                    // echo ' <td><input type="text" name="txt_price[]" id="txt_price_' . $i . '"
                                                    //             onkeyup="cal()"
                                                    //             class="form-control pricefield txt_price txt_price_' . $i . ' numberss text-right"
                                                    //         value="0"  >
                                                    //         <input type="hidden"
                                                    //             class="txt_comm txt_comm_' . $i . '" name="txt_comm" id="txt_comm"
                                                    //             value=""></td>


                                                    //         <td><input type="text" name="txt_final_total[]"
                                                    //                 id="txt_final_total_' . $i . '"
                                                    //                 class="form-control txt_final_total_' . $i . ' numberss txt_final_total text-right"
                                                    //                 readonly></td>

                                                    //     </tr>';



                                                    $i++;
                                                }
                                                $calendarStart = $i;

                                                $calendarRows = [
                                                    'Product (Calendar)',
                                                    'Category (Calendar)',
                                                    'Size (Calendar)',
                                                    'Inner Sheet (Calendar)',
                                                    'Special Effects (Calendar)'
                                                ];

                                                foreach ($calendarRows as $label) {
                                                ?>
                                                    <!-- <tr class="calendar-row">
                                                        <td></td>
                                                        <td></td>

                                                        <td>
                                                            <input type="hidden"
                                                                name="txt_itemtypes[]"
                                                                class="txt_itemtypes txt_itemtypes_<?php echo $calendarStart; ?>"
                                                                data-czid="<?php echo $calendarStart; ?>"
                                                                value="0">
                                                            <?php echo $label; ?>
                                                        </td>

                                                        <td>
                                                            <select class="form-control txt_item txt_item_<?php echo $calendarStart; ?>"
                                                                name="txt_item[]"
                                                                data-czid="<?php echo $calendarStart; ?>"
                                                                <?php echo $isProductCalendar ? 'data-calendar="1"' : ''; ?>>
                                                            </select>
                                                        </td>

                                                        <td>
                                                            <input type="text"
                                                                class="form-control txt_product_qty txt_product_qty_<?php echo $calendarStart; ?>"
                                                                id="txt_product_qty_<?= $calendarStart ?>"
                                                                name="txt_product_qty[]"
                                                                value="0"
                                                                onkeyup="cal()">
                                                        </td>

                                                        <td>
                                                            <select class="form-control txt_price_type txt_price_type_<?php echo $calendarStart; ?>"
                                                                name="txt_price_type[]"
                                                                data-czid="<?php echo $calendarStart; ?>">
                                                            </select>
                                                        </td>

                                                        <td>
                                                            <select class="form-control txt_orientation">
                                                                <option value="">NONE</option>
                                                            </select>
                                                        </td>

                                                        <td>
                                                            <input type="text"
                                                                class="form-control txt_price txt_price_<?php echo $calendarStart; ?>"
                                                                name="txt_price[]"
                                                                value="0"
                                                                onkeyup="cal()">
                                                        </td>

                                                        <td>
                                                            <input type="text"
                                                                class="form-control txt_final_total txt_final_total_<?php echo $calendarStart; ?>"
                                                                name="txt_final_total[]"
                                                                readonly>
                                                        </td>
                                                    </tr> -->
                                                <?php
                                                    $calendarStart++;
                                                }
                                                ?>

                                            </tbody>
                                        </table>
                                    </div>
                                    <div class="colum_split">
                                        <div class="one-half-column">
                                        </div>
                                        <div class="one-half-column pull-right custom_table_widths">
                                            <div class="table-div table-responsive">
                                                <table class="table-bordered table thead amountdetails" width="100%"
                                                    cellspacing="0" cellpadding="0">
                                                    <thead>
                                                        <tr>
                                                            <td class="text-right">
                                                                <label style="margin-top: 6px;"
                                                                    class="agents"><strong>Items
                                                                        Total(₹)</strong></label>
                                                            </td>
                                                            <td>
                                                                <input type="text" name="txt_item_total"
                                                                    class="form-control txt_item_total pull-right w-21 text-right numberss"
                                                                    id="txt_item_total" readonly="">
                                                            </td>
                                                            <!-- <td></td> -->
                                                        </tr>



                                                    </thead>
                                                    <tbody class="totalamounts">
                                                        <tr>
                                                            <td class="text-right">
                                                                <label for="customerPlanLabel">
                                                                    <strong>Customer Plan: Not Selected (0%)</strong>
                                                                </label>
                                                            </td>
                                                            <td class="text-right">
                                                                <input type="text" readonly="" id="discount_amt"
                                                                    name="discount_amt"
                                                                    class="form-control discount_amt pull-right w-21 text-right">
                                                            </td>
                                                        </tr>

                                                        <tr>
                                                            <td class="text-right">
                                                                <label for="customerPlanLabel">
                                                                    <strong>Total(₹)</strong>
                                                                </label>
                                                            </td>
                                                            <td class="text-right">

                                                                <input type="text" name="plan_dis_amt" id="plan_dis_amt"
                                                                    readonly=""
                                                                    class="form-control plan_discount pull-right w-21 text-right"
                                                                    value="">
                                                                <input type="hidden" name="plan_discount"
                                                                    id="plan_discount" value="">
                                                            </td>

                                                        </tr>

                                                        <!-- <tr class="apRow">
                                                            <td class="text-right">
                                                                <span class="totalamounts_sec mt-10 "
                                                                    style="display: flex;align-items: center;justify-content: flex-end;">
                                                                    <label class=""
                                                                        style="    padding-right: 15px;">DISCOUNT</label>
                                                                    <div class="input-group btn-type w-49"
                                                                        style="margin-right: 20px !important;width: 65% !important;">
                                                                        <input type="text" name="discount_final"
                                                                            id="discount_final" onkeyup="cal()"
                                                                            class="form-control igst pull-left discount_final_1"
                                                                            placeholder="%" max="100">
                                                                        <span class="input-group-addon"><i
                                                                                class="fa fa-percent"></i></span>
                                                                    </div>
                                                                    <select
                                                                        class="form-control txt_cal_type_igst pull-left numberss pricefieldchanges extraprices"
                                                                        name="txt_cal_type_igst_final"
                                                                        id="txt_cal_type_igst_final">
                                                                        <option value="1" selected="">-</option>
                                                                    </select>
                                                                </span>
                                                            </td>
                                                            <td>
                                                                <input type="text" name="discount_final_amt"
                                                                    class="form-control txt_field_value_comm totalcalc extrapricescomm pull-right text-right numberss"
                                                                    id="discount_final_amt" readonly="readonly" min="0"
                                                                    max="' + itemTot + '">
                                                            </td>
                                                        </tr>-->

                                                        <tr class="apRow">
                                                            <td class="text-right">
                                                                <span class="totalamounts_sec mt-10 "
                                                                    style="display: flex;align-items: center;justify-content: flex-end;">
                                                                    <label class=""
                                                                        style="width: 39% !important; padding-right: 15px;">
                                                                        <select class="form-control  discount_field1 "
                                                                            name="discount_field1" id="discount_field1">
                                                                            <option value="" selected="">Select one
                                                                            </option>
                                                                            <option value="1"="">Transport</option>
                                                                            <option value="2">Courier</option>
                                                                            <option value="3">Direct</option>
                                                                        </select>

                                                                    </label>

                                                                    <div class="input-group btn-type w-49"
                                                                        style="margin-right: 20px !important;width: 30% !important;">
                                                                        <input type="text" name="discount_final1"
                                                                            id="discount_final1" onkeyup="cal()"
                                                                            class="form-control igst pull-left discount_final1"
                                                                            placeholder="">

                                                                        <select
                                                                            class="form-control txt_cal_type txt_cal_type1 "
                                                                            name="txt_cal_type1" id="txt_cal_type1"
                                                                            onchange="cal()">
                                                                            <option value="1" selected="">=</option>
                                                                            <option value="2">%</option>
                                                                        </select>

                                                                    </div>
                                                                    <select
                                                                        class="form-control discount_type1 pull-left numberss pricefieldchanges extraprices"
                                                                        name="discount_type1" id="discount_type1"
                                                                        onchange="cal()">
                                                                        <option value="1" selected="">+</option>
                                                                        <option value="2">-</option>
                                                                    </select>
                                                                </span>
                                                            </td>
                                                            <td>
                                                                <input type="text" name="discount_final_amt1"
                                                                    class="form-control discount_final_amt1 totalcalc extrapricescomm pull-right text-right numberss"
                                                                    id="discount_final_amt1" readonly="readonly">
                                                            </td>
                                                        </tr>

                                                        <tr class="apRow">
                                                            <td class="text-right">
                                                                <span class="totalamounts_sec mt-10 "
                                                                    style="display: flex;align-items: center;justify-content: flex-end;">
                                                                    <label class=""
                                                                        style="    padding-right: 15px;">Discount<input
                                                                            type="hidden" name="discount_field2"
                                                                            id="discount_field2" onkeyup="cal()"
                                                                            class="form-control  pull-left discount_field2"
                                                                            value="Discount" readonly></label>
                                                                    <div class="input-group btn-type w-49"
                                                                        style="margin-right: 20px !important;width: 30% !important;">
                                                                        <input type="text" name="discount_final2"
                                                                            id="discount_final2" onkeyup="cal()"
                                                                            class="form-control igst pull-left discount_final2"
                                                                            placeholder="">
                                                                        <select
                                                                            class="form-control txt_cal_type txt_cal_type2 "
                                                                            name="txt_cal_type2" id="txt_cal_type2"
                                                                            onchange="cal()">
                                                                            <option value="1" selected="">=</option>
                                                                            <option value="2">%</option>

                                                                        </select>
                                                                    </div>
                                                                    <select
                                                                        class="form-control discount_type2 pull-left numberss pricefieldchanges extraprices"
                                                                        name="discount_type2" id="discount_type2"
                                                                        onchange="cal()">
                                                                        <option value="1" selected="">+</option>
                                                                        <option value="2">-</option>
                                                                    </select>
                                                                </span>
                                                            </td>
                                                            <td>
                                                                <input type="text" name="discount_final_amt2"
                                                                    class="form-control discount_final_amt2 totalcalc extrapricescomm pull-right text-right numberss"
                                                                    id="discount_final_amt2" readonly="readonly">
                                                            </td>
                                                        </tr>

                                                        <tr class="apRow">
                                                            <td class="text-right">
                                                                <span class="totalamounts_sec mt-10 "
                                                                    style="display: flex;align-items: center;justify-content: flex-end;">
                                                                    <label class=""
                                                                        style="    padding-right: 15px;"><input
                                                                            type="text" name="discount_field3"
                                                                            id="discount_field3" onkeyup="cal()"
                                                                            class="form-control  pull-left discount_field3"></label>
                                                                    <div class="input-group btn-type w-49"
                                                                        style="margin-right: 20px !important;width: 30% !important;">
                                                                        <input type="text" name="discount_final3"
                                                                            id="discount_final3" onkeyup="cal()"
                                                                            class="form-control igst pull-left discount_final3"
                                                                            placeholder="">
                                                                        <select
                                                                            class="form-control txt_cal_type txt_cal_type3 "
                                                                            name="txt_cal_type3" id="txt_cal_type3"
                                                                            onchange="cal()">
                                                                            <option value="1" selected="">=</option>
                                                                            <option value="2">%</option>

                                                                        </select>
                                                                    </div>
                                                                    <select
                                                                        class="form-control discount_type3 pull-left numberss pricefieldchanges extraprices"
                                                                        name="discount_type3" id="discount_type3"
                                                                        onchange="cal()">
                                                                        <option value="1" selected="">+</option>
                                                                        <option value="2">-</option>
                                                                    </select>
                                                                </span>
                                                            </td>
                                                            <td>
                                                                <input type="text" name="discount_final_amt3"
                                                                    class="form-control discount_final_amt3 totalcalc extrapricescomm pull-right text-right numberss"
                                                                    id="discount_final_amt3" readonly="readonly">
                                                            </td>
                                                        </tr>
                                                        <tr class="intrast apRow" style="display:none">
                                                            <td class="text-right">
                                                                <span class="totalamounts_sec mt-10 intrast"
                                                                    style="display: flex;align-items: center;justify-content: flex-end;">
                                                                    <label class="" style="padding-right: 15px;">CGST
                                                                    </label>
                                                                    <div class="input-group btn-type w-49"
                                                                        style="margin-right: 20px !important;width: 30% !important;">
                                                                        <input type="text" name="cgst_per" id="cgst_per"
                                                                            onkeyup="cal()"
                                                                            class="form-control igst pull-left cgst_per"
                                                                            placeholder="" max="100" value="9">
                                                                        <span class="input-group-addon"><i
                                                                                class="fa fa-percent"></i></span>
                                                                    </div>
                                                                    <select
                                                                        class="form-control txt_cal_type_cgst pull-left numberss pricefieldchanges extraprices"
                                                                        name="txt_cal_type_cgst_"
                                                                        id="txt_cal_type_cgst">
                                                                        <option value="1" selected="">+</option>
                                                                    </select>

                                                                </span>

                                                            </td>
                                                            <td>
                                                                <span class="col-md-12 p-0 intrast"
                                                                    style="display: block;">

                                                                    <input type="text" name="cgst_total"
                                                                        class="form-control cgst_total totalcalc  pull-right text-right numberss"
                                                                        id="cgst_total" readonly="readonly">
                                                                </span>
                                                            </td>
                                                        </tr>

                                                        <tr class="intrast apRow" style="display:none">
                                                            <td class="text-right">
                                                                <span class="totalamounts_sec mt-10 "
                                                                    style="display: flex;align-items: center;justify-content: flex-end;">
                                                                    <label class=""
                                                                        style="    padding-right: 15px;">SGST
                                                                    </label>
                                                                    <div class="input-group btn-type w-49"
                                                                        style="margin-right: 20px !important;width: 30% !important;">
                                                                        <input type="text" name="sgst_per" id="sgst_per"
                                                                            onkeyup="cal()"
                                                                            class="form-control igst pull-left sgst_per"
                                                                            placeholder="" max="100" value="9">
                                                                        <span class="input-group-addon"><i
                                                                                class="fa fa-percent"></i></span>
                                                                    </div>
                                                                    <select
                                                                        class="form-control txt_cal_type_sgst pull-left numberss pricefieldchanges extraprices"
                                                                        name="txt_cal_type_sgst" id="txt_cal_type_sgst">
                                                                        <option value="1" selected="">+</option>
                                                                        <!-- <option value="2">-</option> -->
                                                                    </select>

                                                                </span>

                                                            </td>
                                                            <td>
                                                                <span class="col-md-12 p-0 " style="display: block;">

                                                                    <input type="text" name="sgst_total"
                                                                        class="form-control sgst_total totalcalc  pull-right text-right numberss"
                                                                        id="sgst_total" readonly="readonly">

                                                                </span>
                                                            </td>
                                                        </tr>
                                                        <tr class="interst apRow">
                                                            <td class="text-right">
                                                                <span class="totalamounts_sec mt-10 "
                                                                    style="display: flex;align-items: center;justify-content: flex-end;">
                                                                    <label class=""
                                                                        style="    padding-right: 15px;">IGST
                                                                    </label>
                                                                    <div class="input-group btn-type w-49"
                                                                        style="margin-right: 20px !important;width: 30% !important;">
                                                                        <input type="text" name="igst_per" id="igst_per"
                                                                            onkeyup="cal()"
                                                                            class="form-control igst pull-left igst_per"
                                                                            placeholder="" max="100" value="18">
                                                                        <span class="input-group-addon"><i
                                                                                class="fa fa-percent"></i></span>
                                                                    </div>
                                                                    <select
                                                                        class="form-control txt_cal_type_igst pull-left numberss pricefieldchanges extraprices"
                                                                        name="txt_cal_type_igst" id="txt_cal_type_igst">
                                                                        <option value="1" selected="">+</option>
                                                                        <!-- <option value="2">-</option> -->
                                                                    </select>

                                                                </span>

                                                            </td>
                                                            <td>
                                                                <span class="col-md-12 p-0 " style="display: block;">

                                                                    <input type="text" name="igst_total"
                                                                        class="form-control igst_total totalcalc  pull-right text-right numberss"
                                                                        id="igst_total" readonly="readonly">

                                                                </span>
                                                            </td>
                                                        </tr>

                                                        <!--   <tr>
                                                            <td align="right">
                                                                <label for="" class="control-label text-right gtotal">
                                                                    <strong> Total(₹)</strong></label>
                                                            </td>
                                                            <td class="text-right">
                                                                <input type="text" name="txt_total"
                                                                    class="form-control txt_total pull-right w-21 text-right"
                                                                    id="txt_total" readonly="" value="0.00">
                                                            </td>

                                                        </tr>-->
                                                        <tr>
                                                            <td align="right">
                                                                <label for="" class="control-label text-right gtotal">
                                                                    <strong>Grand Total(₹)</strong></label>
                                                            </td>
                                                            <td class="text-right">
                                                                <input type="text" name="txt_grand_total"
                                                                    class="form-control txt_grand_total pull-right w-21 text-right"
                                                                    id="txt_grand_total" readonly="" value="0.00">
                                                            </td>
                                                            <!-- <td></td> -->
                                                        </tr>

                                                    </tbody>
                                                </table>
                                            </div>
                                        </div>
                                    </div>

                                </div>
                            </div>
                            <!-- /.card-body -->
                            <div class="card-footer text-right">
                                <input type="hidden" name="deleted" value="0" id="deleted" />
                                <input type="hidden" name="total_product" value="0" id="total_product" />
                                <input type="hidden" name="mode" value="addEstimateNonComm" />
                                <input type="hidden" name="cus_id" id="cus_id">
                                <button type="submit" class="btn_save btn btn-success btn-lg">Save</button>
                            </div>
                        </form>
                    </div>
                    <!-- /.card -->
                </div>
                <!--/.col (left) -->
                <!-- right column -->
                <div class="col-md-6">

                </div>
                <!--/.col (right) -->
            </div>
            <!-- /.row -->
        </div><!-- /.container-fluid -->
    </section>
    <!-- /.content -->
</div>
<style type="text/css">
    /* .theme-form .control-field {
        display: flex;
    } */

    .select2 {
        width: 100% !important;
    }

    .one-half-column {
        width: 60%;
        padding: 0;
        float: left;
    }

    .custom_table_widths {
        width: 40%;
    }

    .table-div {
        clear: left;
    }

    .colum_split {
        display: flex;
    }

    .custom_table_widths .table-div table tbody tr:first-child td .gstcol span.totalamounts_sec {
        display: flex;
        align-items: center;
        justify-content: flex-end;
    }

    .custom_table_widths .table-div table tbody tr:first-child td .gstcol span.totalamounts_sec label {
        padding-right: 15px;
    }

    .custom_table_widths .table-div table tbody tr:first-child td .gstcol span.totalamounts_sec .input-group {
        margin-right: 20px !important;
        width: 65% !important;
    }

    #txt_cal_type_cgst_final,
    #txt_cal_type_sgst_final,
    #txt_cal_type_igst_final {
        width: 15% !important;
        height: 32px !important;
        margin-left: 10px !important;
    }

    input[type="checkbox"],
    input[type="radio"] {
        margin: 4px 0 0;
        margin-top: 1px\9;
        line-height: normal;
    }

    .table .form-control {
        padding: 6px;
    }

    label {
        font-weight: normal;
        text-transform: uppercase;
    }

    label {
        display: inline-block;
        max-width: 100%;
        margin-bottom: 5px;
        font-weight: 700;
    }

    /*
.custom_table_widths .table-div table tbody tr:nth-child(n+2) td:first-child input {
    width: 80% !important;
}
*/
    .custom_table_widths .table-div table tbody tr td:first-child select#txt_cal_types_comm {
        width: 15% !important;
        float: right !important;
    }

    select#txt_cal_types_comm {
        margin-left: 10px !important;
    }

    .class_per {
        text-align: center !important;
        width: 20%;
        float: left;
    }

    .class_amt {
        width: 75%;
        float: left;
        margin-left: 5%;
    }


    .apRow .extraprices {
        width: 15%;
    }

    .bgselectedcolor {
        background: #0066B2;
        color: #ffffff;
    }
</style>

<script>
    $(document).ready(function() {

        // Load normal rows (already working)
        getComercialorNonItemsType(1);
        getComercialorNonItemsType(2);
        getComercialorNonItemsType(3);
        getComercialorNonItemsType(4);
        getComercialorNonItemsType(5);
        getComercialorNonItemsType(6);
        getComercialorNonItemsType(7);
        getComercialorNonItemsType(8);

        <?php
        for ($x = $calendarStart - count($calendarRows); $x < $calendarStart; $x++) {
            echo "getComercialorNonItemsType($x);\n";
        }
        ?>
    });
</script>

<script>
    //getCategoryListing(1);


    $('#form_estimate_noncomm_add').on('keyup keypress', function(e) {
        var keyCode = e.keyCode || e.which;
        if (keyCode === 13) {
            e.preventDefault();
            return false;
        }
    });
    $('#txt_customer_name').select2();
    $(".nav-link").removeClass("active");
    $(".nav-item").removeClass("menu-open");

    $(".sales").addClass("menu-open");
    $(".sales_customer .nav-link").addClass("active");



    getCity();


    $('table').on("change", ".txt_item", function(e) {
        if ($(this).val() > 0) {
            $(this).addClass('bgselectedcolor')
        }
    });

    $('table').on("change", ".txt_types", function(e) {
        var $tr = $(this).closest('tr');
        var czid = $(this).attr('data-czid');
        var types = $tr.find(".txt_types_" + czid).find("option:selected").val();
        getCostTypeListing(czid);

        if (types == 1) {
            $tr.find('.txt_itemtypes_' + czid).html('<option selected >Select One</option><option value="0">Product</option>');
        } else if (types == 2) {
            getTypesListing(czid);
        }
        $("#additems").show();
        $tr.find('.txt_product_qty').val(1);

    });

    var deleted = 0;

    $('table').on("click", ".removeitems", function(e) {
        e.preventDefault();
        var rowCount = $('.itemTable >tbody >tr').length;

        if ($('.removeitems').length > 0) {
            var sqpid = $(this).closest('tr').find('td .txt_sqp_id').val();
            if (sqpid) {
                var sqvals = $('.txt_deleted_sqp').val();
                if (sqvals != 0) {
                    var vallss = sqvals + '##' + sqpid;
                } else {
                    var vallss = sqpid;
                }
                $('.txt_deleted_sqp').val(vallss);
            }
            $(this).closest('tr').remove();

            var cusId = $('#txt_customer_name').val();

            var prodid = $('.txt_product_name').val();

            deleted++;
            $('#deleted').val(deleted)
            cal()

        }
    });

    $('table').on("change", ".txt_itemtypes ", function(e) {
        var czid = $(this).attr('data-czid');
        var itemtypes = $('.txt_itemtypes_' + czid).find("option:selected").val();
        var types = $('.txt_types_' + czid).find("option:selected").val();
        if (types == 2) {
            if (itemtypes == 7) {
                getCategoryListing(czid);
            } else {
                $('.txt_category_' + czid).html('<option selected >None</option>');

            }
        } else {
            $('.txt_category_' + czid).html('<option selected >None</option>');

        }
        getComercialorNonItemsType(czid);
    });

    $('table').on("change", ".txt_item, .txt_price_type", function(e) {
        var $tr = $(this).closest('tr');
        var group_id = $tr.data("group_id");
        var czid = $(this).attr("data-czid");
        if (czid == 2) {
            var czidval = $(this).find("option:selected").val();
            $(`.prod_group_${group_id} .txt_price_type`).each(function() {
                var ptczid = $(this).attr("data-czid");
                if (ptczid == 1 || ptczid == 2) {
                    // skip
                } else {
                    $(this).find(`option[value="${czidval}"]`).attr("selected", true);
                    $(this).trigger('change');
                }
            });
        }

        // alert(czid)
        getCostListing(czid, $tr);

    });

    $('.txt_state').on('change', function() {

        var getintval = $(this).find("option:selected").attr('data-code');
        $('.interst').hide();
        $('.intrast').hide();

        if (getintval == 33) {
            $('.intrast').show();
            $('.intrast').find('input').prop("disabled", false);
            $('.interst').find('input').prop("disabled", true);
        } else {
            //$('#gst_per').val(' ');
            $('.interst').show();
            $('.interst').find('input').prop("disabled", false);
            $('.intrast').find('input').prop("disabled", true);

        }
        cal();
    });

    getCostTypeListing(1);

    function getCostListing(czid, $tr) {
        // alert(czid + " || " + $tr.data('group_id'))
        var itemSelect = $tr.find('.txt_item_' + czid);
        var costtype = $tr.find('.txt_price_type_' + czid).find("option:selected").val();
        var typesval = $tr.find('.txt_itemtypes_' + czid).val();
        var itemid = itemSelect.find("option:selected").val();

        // console.log("getCostListing :", typesval, " | ", costtype, " | ", itemid)

        // if (typesval === '2' || typesval === '5' || typesval === '6' || typesval === '7' || typesval === '8') {
        if (['1', '2', '3', '4', '5', '6', '7', '8'].includes(typesval)) {
            // Check if 'SELECT ONE' is selected
            if (itemSelect.val() === "") {
                $tr.find(`.txt_product_qty_${czid}`).val('0');
            } else if ($tr.find(`.txt_product_qty_${czid}`).val() == '0') {
                $tr.find(`.txt_product_qty_${czid}`).val('1');
            }
        }

        // Continue with AJAX request
        $.ajax({
            url: "modules/sales/ajax_functions_noncommercial.php",
            data: {
                'mode': 'getCostListing',
                'typesid': typesval,
                'costtype': costtype,
                'itemtypeId': itemid
            },
            type: 'post',
            dataType: 'json',
            success: function(response) {
                var priceField = $tr.find(`.txt_price_` + czid);

                // Set the price field value
                priceField.val(response);

                // Calculate the total
                cal();
            },
            error: function(response) {
                console.error(response.error);
            }
        });
    }

    $('table').on("change", ".txt_innersheet,.txt_specialeffects,.txt_size", function(e) {
        var czid = $(this).attr("data-czid");

        var incost = $('.txt_innersheet_' + czid).find("option:selected").attr("data-cost");
        var spcost = $('.txt_specialeffects_' + czid).find("option:selected").attr("data-secost");
        var sizecost = $('.txt_size_' + czid).find("option:selected").attr("data-sizecost");


        totcost = 0;
        if (incost) {
            totcost += parseFloat(incost);
        }
        if (spcost) {
            totcost += parseFloat(spcost);
        }
        if (sizecost) {
            totcost += parseFloat(sizecost);
        }


        $('.txt_price_' + czid).val(totcost);


        cal();

    });

    var e = 0;
    $('.addextraitemscomm').on('click', function() {
        var itemTot = $("#txt_item_total").val();
        var tcont = $('tbody.totalamounts tr').length;
        if (tcont > 2) {
            var evalues = parseInt(tcont) - 2;
            e = evalues;
        }
        $('table.amountdetails tbody.totalamounts').find('tr:last').prev().after(
            '<tr class="apRow"><td><button type="button" class="btn btn-danger pull-left removeextraitems"><i class="fa fa-minus" aria-hidden="true"></i></button></td><td class="text-right"><input type="text" name="txt_field_name_comm[]" class="form-control txt_field_name_comm pull-left" id="txt_field_name_comm_' +
            e +
            '" placeholder="Field Name"><select class="form-control txt_cal_types_comm pull-left numbersOnly extrapricescomm chosen" name="txt_cal_types_comm[]" id="txt_cal_types_comm"><option value="1" selected>+</option><option value="2">-</option></select></td><td><input type="text" name="txt_field_value_comm[]" class="form-control txt_field_value_comm totalcalc extrapricescomm pull-right text-right numberss" id="txt_field_value_comm" min="0" max="' +
            itemTot +
            '"></td></tr>'
        );
        $('.extrapricescomm').on('focusout change', function() {
            calculateSCitems();
        });
        getAllExtraCharges(e);
        validatefunctions();
    });

    var f = 0;
    $('.addextraitemscommedit').on('click', function() {
        var itemTot = $("#txt_item_total").val();
        var tcont = $('tbody.totalamountsedit tr').length;
        if (tcont > 2) {
            var fvalues = parseInt(tcont) - 2;
            f = fvalues;
        }
        $('table.amountdetailsedit tbody.totalamountsedit').find('tr:last').prev().after(
            '<tr class="apRow"><td><button type="button" class="btn btn-danger pull-left removeextraitems"><i class="fa fa-minus" aria-hidden="true"></i></button></td><td class="text-right"><input type="text" name="txt_field_name_comm[]" class="form-control txt_field_name_comm pull-left" id="txt_field_name_comm_' +
            f +
            '" placeholder="Field Name"><select class="form-control txt_cal_types_comm pull-left numbersOnly extrapricescomm chosen" name="txt_cal_types_comm[]" id="txt_cal_types_comm"><option value="1" selected>+</option><option value="2">-</option></select></td><td><input type="text" name="txt_field_value_comm[]" class="form-control txt_field_value_comm totalcalc extrapricescomm pull-right text-right numberss" id="txt_field_value_comm" min="0" max="' +
            itemTot +
            '"></td></tr>'
        );

        $('.extrapricescomm').on('focusout change', function() {
            calculateSCitems();
        });
        getAllExtraCharges(f);
        validatefunctions();
    });

    $('table').on("click", ".removeextraitems", function(e) {
        e.preventDefault();
        $(this).closest('tr').remove();
        calculateSCitems();
    });

    getCostTypeListing(1);

    function getCostTypeListing(types, group_id = null) {

        if (group_id) {
            var $tr = $(`.itemTable >tbody >tr[data-group_id="${group_id}"]`);
            $tr.find('.txt_price_type_' + types).html('<option value="" selected >SELECT ONE </option>');

            $tr.find('.txt_price_type_' + types).append(
                '<option value="1">Price (or) 4 Color</option><option value="2">Price (or) 7 Color</option><option value="0">NONE</option>'
            );
        } else {
            $('.txt_price_type_' + types).html('<option value="" selected >SELECT ONE </option>');

            $('.txt_price_type_' + types).append(
                '<option value="1">Price (or) 4 Color</option><option value="2">Price (or) 7 Color</option><option value="0">NONE</option>'
            );
        }


    }

    function getCity() {
        $('#txt_customer_city').empty();
        $('#txt_customer_city').append('<option value=""  selected>SELECT CITY</option>');
        $.post("modules/sales/ajax_functions_noncommercial.php", {
                mode: 'getAllCities',
            },
            function(data, status) {
                var data = jQuery.parseJSON(data);
                var cityNameOpt = '';
                if (data.length > 0) {
                    var cityNameOpt = '';
                    for (var i in data) {

                        cityNameOpt = cityNameOpt + '<option value=' + data[i]['pk_city_id'] + '>' + data[i]['city'] +
                            '</option>';
                    }
                    $('#txt_customer_city').append(cityNameOpt);
                }

            });

    }

    function getTypesListing(type) {
        // alert('test');
        $.ajax({
            url: "modules/products/ajax_functions_noncommercial.php",
            data: {
                'mode': 'getTypesListing'
            },
            type: 'post',
            dataType: 'json',
            success: function(response) {
                $('.txt_itemtypes_' + type).html('<option selected >Select</option>');
                for (var i = 0; i < response[0].length; i++) {
                    if (response[0][i].pk_types_id == '9999') {
                        //  type_tables, table_pk_id, orderid
                        $('.txt_itemtypes_' + type).append('<option value="' + response[0][i]
                            .pk_types_id + '" data-types="2" data-tables="' + response[0][i].type_tables +
                            '" data-pkid="' + response[0][i].table_pk_id + '">' + response[0][i]
                            .types_name + ' </option>');
                    } else {

                        $('.txt_itemtypes_' + type).append('<option value="' + response[0][i].pk_types_id +
                            '" data-types="1" data-tables="' + response[0][i].type_tables +
                            '" data-pkid="' + response[0][i].table_pk_id + '">' + response[0][i]
                            .types_name + '</option>');
                    }
                }


            },
            error: function(response) {
                console.log(response);
            }
        });
    }

    function afterProductLoaded(type) {

        var $product = $('.txt_item_' + type);

        if ($product.data('calendar') == 1) {
            $product.find('option').each(function() {
                if ($(this).text().trim().toLowerCase() === 'calendar') {
                    $(this).prop('selected', true).trigger('change');
                }
            });

            // lock ONLY product(calendar)
            $product.prop('disabled', true);
        }
    }



    getComercialorNonItemsType(1);
    getComercialorNonItemsType(2);
    getComercialorNonItemsType(3);
    getComercialorNonItemsType(4);
    getComercialorNonItemsType(5);
    getComercialorNonItemsType(6);
    getComercialorNonItemsType(7);
    getComercialorNonItemsType(8);


    function getComercialorNonItemsType(type, group_id = null) {
        // alert('all')
        var typesid = 2;
        var itemtypeId = type;

        // parent_id = $('.txt_item_' + type).find("option:selected").attr('data-parent_id');
        var parent_id = 0;

        var $tr = null;
        if (group_id) {
            $tr = $(`.itemTable >tbody >tr[data-group_id="${group_id}"]`);
            if (type == 2) {
                parent_id = $tr.find('.txt_item_1').find("option:selected").attr('data-parent_id');
            }
        } else {
            if (type == 2) {
                parent_id = $('.txt_item_1').find("option:selected").attr('data-parent_id');
            }
        }

        $.ajax({
            url: "modules/sales/ajax_functions_noncommercial.php",
            data: {
                'mode': 'getComercialorNonItemsType',
                'typesid': typesid,
                'itemtypeId': itemtypeId,
                'parent_id': parent_id
            },
            type: 'post',
            dataType: 'json',
            success: function(response) {
                if (group_id && $tr) {
                    $tr.find(`.txt_item_${type}`).html('<option value="" selected>SELECT ONE</option>');
                } else {
                    $(`.txt_item_${type}`).html('<option value="" selected>SELECT ONE</option>');
                }

                $('.itemsdata').show();

                // console.log("ids:", type, group_id)

                // for (var i = 0; i < response[0].length; i++) {
                response[0].forEach((row, index) => {
                    if (group_id && $tr) {
                        if (row.pk_items_id == '9999') {
                            //  type_tables, table_pk_id, orderid
                            $tr.find(`.txt_item_${type}`).append(`<option value="${row.pk_items_id}"  data-items="2" data-parent_id="${row.parent_id}">${row.fk_item_id}</option>`);
                        } else {
                            $tr.find(`.txt_item_${type}`).append(`<option value="${row.pk_items_id}"  data-items="1" data-parent_id="${row.parent_id}">${row.fk_item_id}</option>`);
                        }
                    } else {
                        if (row.pk_items_id == '9999') {
                            //  type_tables, table_pk_id, orderid
                            $(`.txt_item_${type}`).append(`<option value="${row.pk_items_id}"  data-items="2" data-parent_id="${row.parent_id}">${row.fk_item_id}</option>`);
                        } else {
                            $(`.txt_item_${type}`).append(`<option value="${row.pk_items_id}"  data-items="1" data-parent_id="${row.parent_id}">${row.fk_item_id}</option>`);
                        }
                    }
                })
                afterProductLoaded(type);

            },

            error: function(response) {
                console.error(response.error);
            }

        });

        getCostTypeListing(type, group_id);

    }


    $(document).on('change', '.txt_item_1', function() {
        var group_id = $(this).closest('tr').data("group_id");
        var parent_id = $(this).find("option:selected").val();
        getComercialorNonItemsTypeCategory(2, parent_id, group_id);
    });



    function getComercialorNonItemsTypeCategory(type, parent_id, group_id) {
        // console.log('Check :', type, parent_id);
        var typesid = 2;
        var itemtypeId = type;

        $.ajax({
            url: "modules/sales/ajax_functions_noncommercial.php",
            data: {
                'mode': 'getComercialorNonItemsType',
                'typesid': typesid,
                'itemtypeId': itemtypeId,
                'parent_id': parent_id
            },
            type: 'post',
            dataType: 'json',
            success: function(response) {
                $(`.prod_group_${group_id} .txt_item_${type}`).html('<option value="" selected>SELECT ONE</option>');
                $('.itemsdata').show();
                for (var i = 0; i < response[0].length; i++) {
                    if (response[0][i].pk_items_id == '9999') {
                        //  type_tables, table_pk_id, orderid
                        $(`.prod_group_${group_id} .txt_item_${type}`).append('<option value="' + response[0][i]
                            .pk_items_id + '"  data-items="2" data-parent_id="' + response[0][i].parent_id +
                            '">' + response[0][i].fk_item_id +
                            ' </option>');
                    } else {

                        $(`.prod_group_${group_id} .txt_item_${type}`).append('<option value="' + response[0][i]
                            .pk_items_id + '"  data-items="1" data-parent_id="' + response[0][i].parent_id +
                            '">' + response[0][i].fk_item_id +
                            '</option>');
                    }
                }


            },
            error: function(response) {
                console.log(response);
            }
        });

        getCostTypeListing(type, group_id);

    }

    function getProductListing(type) {
        //alert('test');
        $.ajax({
            url: "modules/products/ajax_functions_noncommercial.php",
            data: {
                'mode': 'getProductListing'
            },
            type: 'post',
            dataType: 'json',
            success: function(response) {
                console.log(response);
                //$('.txt_product_name').chosen('destroy');
                $('.txt_product_name_' + type).html('');
                $('.txt_product_name_' + type).html('<option  value="" selected disabled>Select One</option>');
                for (var i = 0; i < response[0].length; i++) {
                    if (response[0][i].pk_product_id == '9999') {

                        $('.txt_product_name_' + type).append('<option value="' + response[0][i]
                            .pk_product_id + '" data-types="2">' + response[0][i].product_name +
                            ' </option>');
                    } else {

                        $('.txt_product_name_' + type).append('<option value="' + response[0][i]
                            .pk_product_id + '" data-types="1">' + response[0][i].product_name +
                            '</option>');
                    }
                }


            },
            error: function(response) {
                console.log(response);
            }
        });
    }

    function getCategoryListing(type) {
        $.ajax({
            url: "modules/sales/ajax_functions_noncommercial.php",
            data: {
                'mode': 'getCategoryListing'
            },
            type: 'post',
            dataType: 'json',
            success: function(response) {

                //$('.txt_product_name').chosen('destroy');
                $('.txt_category_' + type).html('');
                $('.txt_category_' + type).html('<option value="" selected disabled>Select</option>');
                for (var i = 0; i < response[0].length; i++) {
                    if (response[0][i].pk_product_id == '9999') {

                        $('.txt_category_' + type).append('<option value="' + response[0][i]
                            .pk_cat_id + '" data-types="2">' + response[0][i].cat_name + '</option>');
                    } else {

                        $('.txt_category_' + type).append('<option value="' + response[0][i]
                            .pk_cat_id + '" data-types="1">' +
                            response[0][i].cat_name + '</option>');
                    }
                }


            },
            error: function(response) {
                console.log(response);
            }
        });
    }

    function addrow() {
        var cusId = $('#txt_customer_name').val();
        var tcont = $('tbody.itemclone tr').length;
        var deleted = $('#deleted').val();
        zz = parseInt(tcont) + 1 + parseInt(deleted);
        console.log(zz);
        $('.nodata').remove();


        $('table .itemclone').append('<tr><td><select class="form-control txt_types txt_types_' + zz +
            ' " name="txt_types[]" id="txt_types_' + zz + '" data-czid="' + zz +
            '"   > <option value="">Select Types</option><option value="1">Commercial</option><option value="2">Non Commercial</option></select></td> <td><select class="form-control txt_itemtypes txt_itemtypes_' +
            zz + ' " name="txt_itemtypes[]" id="txt_itemtypes_' + zz + '"  data-czid="' + zz +
            '" data-classids="txt_itemtypes_' + zz +
            '" onchange="cal()"></select></td><td ><select  class="form-control txt_item  txt_item_' + zz +
            '" name="txt_item[]" id="txt_item_' + zz + '" data-czid="' + zz +
            '"   onchange="cal()"></select></td><td><select class="form-control txt_category txt_category_' +
            zz +
            ' "  name="txt_category[]" id="txt_category_' + zz +
            '"   ></select></td><td><input  class="form-control txt_product_qty numbersOnly  txt_product_qty_' +
            zz + '" min="0" max="999999" id="txt_product_qty_' + zz +
            '" name="txt_product_qty[]" placeholder="Quantity"  onkeyup="cal()"  value="1"></td><td><select class="form-control txt_price_type txt_price_type_' +
            zz + ' " data-czid="' + zz + '"  name="txt_price_type[]" id="txt_price_type_' + zz +
            '"  ></select></td><td><select class="form-control txt_orientation txt_orientation_' + zz +
            '"  name="txt_orientation[]" id="txt_orientation_' + zz +
            '"   ><option value="">Select Orientation</option><option value="1">Length</option><option value="2">Breadth </option><option value="">NONE </option></select></td><td ><input type="text" name="txt_price[]" id="txt_price_' +
            zz + '" class="form-control pricefield txt_price txt_price_' + zz +
            ' numberss text-right" onkeyup="cal()"  value="0" ><input type="hidden" class="txt_comm txt_comm_' + zz +
            '" name="txt_comm" id="txt_comm" value=""></td><td ><input type="text" name="txt_final_total[]" id="txt_final_total_' +
            zz + '" class="form-control txt_final_total_' + zz +
            ' numberss txt_final_total text-right"  readonly></td><td><button type="button" name="removeitems" id="removeitems" class="btn btn-danger removeitems" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></button></td></tr>'
        );
        //getTypesListing(zz);
        //getProductListing(zz);
        //getCategoryListing(zz);
        var types = $('.txt_types_' + zz).find("option:selected").val();
        $('.txt_price_type_' + zz).trigger('change');
        if (types == 2) {
            getTypesListing(zz);
        } else {
            $('.txt_itemtypes_' + zz).html('<option selected >Select One</option><option value="1">Product</option>');

        }


    }



    function addinnersheetrow(group_id) {
        var cusId = $('#txt_customer_name').val();
        var tcont = $('tbody.itemclone tr').length;
        var deleted = $('#deleted').val();
        console.log(tcont);
        console.log(deleted);
        zz = parseInt(tcont) + 1 + parseInt(deleted);
        console.log(zz);



        $('.nodata').remove();


        $(`table .itemclone .addinnercls.prod_group_${group_id}`).after(
            '<tr class="prod_group_' + group_id + ' row_group item_type_4" data-group_id="' + group_id + '.1" data-type_id="4"><td><input type="hidden" name="group_id[]" value="' + group_id + '"></td><td><button type="button" name="removeitems" id="removeitems" class="btn btn-danger removeitems" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></button></td><td><input type="hidden" class="form-control txt_itemtypes txt_itemtypes_' +
            zz + '"   name="txt_itemtypes[]" id="txt_itemtypes_' + zz + '"     data-czid="' + zz +
            '" data-classids="txt_itemtypes_' + zz +
            '" value="4" >Inner Sheet</td><td ><select  class="form-control txt_item  txt_item_' + zz +
            '" name="txt_item[]" id="txt_item_' + zz + '" data-czid="' + zz +
            '"   onchange="cal()"></select></td><td><input onkeyup="cal()"  class="form-control txt_product_qty numbersOnly  txt_product_qty_' +
            zz + '" min="0" max="999999" id="txt_product_qty_' + zz +
            '" name="txt_product_qty[]" placeholder="Quantity"  value="1"></td><td><select class="form-control txt_price_type txt_price_type_' +
            zz + ' " data-czid="' + zz + '"  name="txt_price_type[]" id="txt_price_type_' + zz +
            '"  ></select></td><td><select class="form-control txt_orientation txt_orientation_' + zz +
            '"  name="txt_orientation[]" id="txt_orientation_' + zz +
            '"   ><option value="">NONE </option></select></td><td ><input type="text" name="txt_price[]" id="txt_price_' +
            zz + '"  class="form-control pricefield txt_price txt_price_' + zz +
            ' numberss text-right" onkeyup="cal()" value="0" ><input type="hidden" class="txt_comm txt_comm_' + zz +
            '" name="txt_comm" id="txt_comm" value=""></td><td ><input type="text" name="txt_final_total[]" id="txt_final_total_' +
            zz + '" class="form-control txt_final_total_' + zz +
            ' numberss txt_final_total text-right"  value="0.00" readonly></td></tr>'
        );



        getComercialorNonInnerItemsType(zz, 4);
        getCostTypeListing(zz);
        var prtypeval = $('#txt_price_type_2').find("option:selected").val();
        $('.txt_price_type_' + zz + '').find('option[value="' + prtypeval + '"]').attr("selected", true);
        $('.txt_price_type_' + zz + '').trigger('change');

    }

    function addspecialeffectrow(group_id) {
        var cusId = $('#txt_customer_name').val();
        var tcont = $('tbody.itemclone tr').length;
        var deleted = $('#deleted').val();
        zz = parseInt(tcont) + 1 + parseInt(deleted);
        $('.nodata').remove();


        $(`table .itemclone .addsplcls.prod_group_${group_id}`).after(
            '<tr class="prod_group_' + group_id + ' row_group item_type_5"  data-group_id="' + group_id + '" data-type_id="5"><td><input type="hidden" name="group_id[]" value="' + group_id + '"></td><td><button type="button" name="removeitems" id="removeitems" class="btn btn-danger removeitems" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></button></td><td><input type="hidden" class="form-control txt_itemtypes txt_itemtypes_' +
            zz + '"   name="txt_itemtypes[]" id="txt_itemtypes_' + zz + '"    data-czid="' + zz +
            '" data-classids="txt_itemtypes_' + zz +
            '" value="5">Special Effects</td><td ><select  class="form-control txt_item  txt_item_' + zz +
            '" name="txt_item[]" id="txt_item_' + zz + '" data-czid="' + zz +
            '"  onchange="cal()"></select></td><td><input  class="form-control txt_product_qty numbersOnly  txt_product_qty_' +
            zz + '" min="0" max="999999" id="txt_product_qty_' + zz +
            '" name="txt_product_qty[]" onkeyup="cal()"  placeholder="Quantity"  value="1"></td><td><select class="form-control txt_price_type txt_price_type_' +
            zz + ' " data-czid="' + zz + '"  name="txt_price_type[]" id="txt_price_type_' + zz +
            '" ></select></td><td><select class="form-control txt_orientation txt_orientation_' + zz +
            '"  name="txt_orientation[]" id="txt_orientation_' + zz +
            '"   ><option value="">NONE </option></select></td><td ><input type="text" name="txt_price[]" id="txt_price_' +
            zz + '"  class="form-control pricefield txt_price txt_price_' + zz +
            ' numberss text-right" onkeyup="cal()" value="0" ><input type="hidden" class="txt_comm txt_comm_' + zz +
            '" name="txt_comm" id="txt_comm" value=""></td><td ><input type="text" name="txt_final_total[]" id="txt_final_total_' +
            zz + '" class="form-control txt_final_total_' + zz +
            ' numberss txt_final_total text-right"  value="0.00" readonly></td></tr>'
        );

        getComercialorNonInnerItemsType(zz, 5);
        getCostTypeListing(zz);
        var prtypeval = $('#txt_price_type_2').find("option:selected").val();
        $('.txt_price_type_' + zz + '').find('option[value="' + prtypeval + '"]').attr("selected", true);
        $('.txt_price_type_' + zz + '').trigger('change');

    }

    function addProductGroup() {
        // Find the highest existing group number
        let lastGroupNum = 0;
        $('[class*="prod_group_"]').each(function() {
            const match = this.className.match(/prod_group_(\d+)/);
            if (match) {
                const num = parseInt(match[1]);
                if (num > lastGroupNum) lastGroupNum = num;
            }
        });

        const newGroupNum = lastGroupNum + 1;

        $("#total_product").val(newGroupNum)

        // Clone all rows of the last group
        const $lastGroupRows = $(`.prod_group_${lastGroupNum}`);

        $.ajax({
            url: "modules/sales/ajax_functions_noncommercial.php",
            method: "POST",
            data: {
                mode: "getAllTypes"
            },
            async: false,
            dataType: "JSON",
            success: function(response) {

                var html = '';

                for (let index = 0; index < response.length; index++) {
                    const row = response[index];
                    const type = parseInt(row['pk_types_id']);

                    if ([6, 7, 8].includes(type)) {
                        continue;
                    }

                    var prodGroupBtn = '';
                    var hideorient = '';
                    var addinnerdata = '';

                    var innersplattr = '';
                    var innersplcls = '';
                    var splcls = '';

                    if (row['pk_types_id'] == 1) {
                        prodGroupBtn = (`
                            <span class="btn btn-sm btn-primary me-2-custom" style="cursor:auto">#${newGroupNum}</span>
                            <button type="button" class="btn btn-success btn-number me-2-custom" data-type="plus" data-field="quant[2]" onclick="addProductGroup()"> 
                                <span class="fa fa-plus"></span>
                            </button>
                            <button type="button" class="btn btn-danger deleteGroupBtn" onclick="deleteProductGroup(${newGroupNum})">
                                <span class="fa fa-trash"></span>
                            </button>
                        `);

                        hideorient = (`
                            <option value="">Select Orientation</option>
                            <option value="1">Length</option>
                            <option value="2">Breadth </option>
                            <option value="">NONE </option>
                        `);
                    } else {
                        hideorient = '<option value="">NONE </option>';
                    }


                    if (row['pk_types_id'] == 4) {
                        innersplattr = `onclick="addinnersheetrow(${newGroupNum})"`;
                        innersplcls = 'addinnercls';
                    } else if (row['pk_types_id'] == 5) {
                        innersplattr = `onclick="addspecialeffectrow(${newGroupNum})"`;
                        innersplcls = 'addsplcls';
                    }

                    if (row['pk_types_id'] == 4 || row['pk_types_id'] == 5) {
                        addinnerdata = (`
                            <button 
                                type="button" 
                                class="btn btn-success btn-number" 
                                ${innersplattr}
                                data-type="plus" 
                                data-field="quant[2]"> 
                                <span class="fa fa-plus"></span>
                            </button>
                        `);
                    }

                    html += (`
                        <tr class="${innersplcls} prod_group_${newGroupNum} row_group item_type_${type}" data-group_id=${newGroupNum}  data-type_id=${type}>
                            <td width="100">${prodGroupBtn}</td>
                            <td width="50">${addinnerdata}</td>
                            <td>
                                <input 
                                    type="hidden" 
                                    class="form-control txt_itemtypes txt_itemtypes_${type} "   
                                    id="txt_itemtypes_${type}_${newGroupNum}"
                                    name="txt_itemtypes[]" 
                                    value="${row['pk_types_id']}"
                                    data-czid="${type}" 
                                    data-classids="txt_itemtypes_${type}" 
                                >
                                ${row['types_name']}
                            </td>
                            <td>
                                <select class="form-control txt_item txt_item_${type}" 
                                    id="txt_item_${type}_${newGroupNum}"
                                    name="txt_item[]"
                                    data-czid="${type}"
                                    ${row['types_name']=== 'Product (Calendar)' ? `data-calendar='1'` :''}>
                                    <!--ajax-->
                                </select>
                            </td>
                            <td>
                                <input 
                                    class="form-control txt_product_qty numbersOnly  txt_product_qty_${type}"
                                    id="txt_product_qty_${type}_${newGroupNum}"
                                    name="txt_product_qty[]" 
                                    value="0"
                                    placeholder="Quantity"
                                    min="0" 
                                    max="999999" 
                                    onkeyup="cal()"
                                >
                            </td>
                            <td>
                                <select class="form-control txt_price_type txt_price_type_${type}"
                                    id="txt_price_type_${type}_${newGroupNum}"
                                    name="txt_price_type[]" 
                                    data-czid="${type}">
                                    <!--ajax-->
                                </select>
                            </td>
                            <td>
                                <select class="form-control txt_orientation txt_orientation_${type}"
                                    id="txt_orientation_${type}_${newGroupNum}"  
                                    name="txt_orientation[]" 
                                    onchange="cal()" >
                                    ${hideorient}
                                </select>
                            </td>
                            <td>
                                <input 
                                    type="text" 
                                    class="form-control pricefield txt_price txt_price_${type} numberss text-right"
                                    id="txt_price_${type}_${newGroupNum}"
                                    name="txt_price[]" 
                                    value="0"
                                    onkeyup="cal()"
                                >
                                <input 
                                    type="hidden"
                                    class="txt_comm txt_comm_${type}" 
                                    id="txt_comm_${newGroupNum}"
                                    name="txt_comm" 
                                    value=""
                                >
                                <input 
                                    type="hidden"
                                    name="group_id[]" 
                                    value="${newGroupNum}"
                                >
                            </td>
                            <td>
                                <input 
                                    type="text" 
                                    class="form-control txt_final_total_${type} numberss txt_final_total text-right"
                                    id="txt_final_total_${type}_${newGroupNum}"
                                    name="txt_final_total[]"
                                    readonly
                                >
                            </td>
                        <tr>
                    `);

                    // Append after the last row of the previous group
                }

                $lastGroupRows.last().after(html);

                getComercialorNonItemsType(1, newGroupNum);
                getComercialorNonItemsType(2, newGroupNum);
                getComercialorNonItemsType(3, newGroupNum);
                getComercialorNonItemsType(4, newGroupNum);
                getComercialorNonItemsType(5, newGroupNum);
                getComercialorNonItemsType(6, newGroupNum);
                getComercialorNonItemsType(7, newGroupNum);
                getComercialorNonItemsType(8, newGroupNum);

            },
        });


    }

    // Delete a group
    function deleteProductGroup(groupNum) {
        if (confirm("Are you sure you want to delete this group?")) {
            $("#total_product").val(parseInt($("#total_product").val()) - 1)
            $(`.prod_group_${groupNum}`).remove();
            cal()
        }
    }

    function getComercialorNonInnerItemsType(type, innerid) {
        // alert('test');
        var typesid = 2;
        var itemtypeId = innerid;
        //   var pkid = $('.txt_itemtypes_' + type).find("option:selected").attr('data-pkid');


        $.ajax({
            url: "modules/sales/ajax_functions_noncommercial.php",
            data: {
                'mode': 'getComercialorNonItemsType',
                'typesid': typesid,
                'itemtypeId': itemtypeId,
                'parent_id': 0
            },
            type: 'post',
            dataType: 'json',
            success: function(response) {
                $('.txt_item_' + type).html('<option value="" selected >SELECT ONE</option>');
                $('.itemsdata').show();
                for (var i = 0; i < response[0].length; i++) {
                    if (response[0][i].pk_items_id == '9999') {
                        //  type_tables, table_pk_id, orderid
                        $('.txt_item_' + type).append('<option value="' + response[0][i]
                            .pk_items_id + '"  data-items="2">' + response[0][i].fk_item_id +
                            ' </option>');
                    } else {

                        $('.txt_item_' + type).append('<option value="' + response[0][i]
                            .pk_items_id + '"  data-items="1" >' + response[0][i].fk_item_id +
                            '</option>');
                    }
                }


            },
            error: function(response) {
                console.log(response);
            }
        });
    }

    function calAdvance(e) {
        var grandtotal = $('#txt_grand_total').val();
        var advance = $('#discount_final4').val();
        $('#discount_final_amt4').val(parseFloat(advance).toFixed(2));
        var pendamt = 0;
        if (parseFloat(advance) > 0) {
            pendamt = parseFloat(grandtotal) - parseFloat(advance);
        } else {
            pendamt = parseFloat(grandtotal);
        }

        $('#discount_final_amt5').val(parseFloat(pendamt).toFixed(2));
    }

    function cal() {

        var deleted = $('#deleted').val();
        var rowCount = $('.itemTable >tbody >tr').length;
        var total_length = parseFloat(rowCount) + parseFloat(deleted);
        var total_total_amount = 0;
        var total_amount = 0;
        var cgst_amttot = 0;
        var sgst_amttot = 0;
        var igst_amttot = 0;
        var total_amounts = 0;
        var plan_discount = parseFloat($('#plan_discount').val()) || 0;
        // console.log(plan_discount);

        console.log("total_length", total_length)

        $('.itemTable >tbody >tr').each(function(index, row) {

            var $tr = $(row);
            var type_id = $tr.data('type_id');

            var taxable_total = 0;
            var qty = $tr.find(".txt_product_qty").val();
            if (parseFloat(qty) > 0) {
                qty = qty;
            } else {
                if (type_id == 1 || type_id == 2 || type_id == 3) {
                    qty = 1;

                } else {
                    qty = 0;
                }
            }

            var price = $tr.find(".txt_price").val();
            if (parseFloat(price) > 0) {
                price = price;
            } else {
                price = 0;
            }
            taxable_total = parseFloat(qty) * parseFloat(price);
            if (parseFloat(taxable_total) > 0) {} else {
                rice = 0;
            }

            taxable_total = parseFloat(qty) * parseFloat(price);

            if (parseFloat(taxable_total) > 0) {} else {
                total_total_amount = total_total_amount + 0;
                $tr.find(".txt_final_total").val("");
            }

            var total_amount = parseFloat(taxable_total);
            total_amounts += taxable_total;
            total_total_amount = total_total_amount + total_amount;

            $tr.find(".txt_final_total").val(parseFloat(total_amount).toFixed(2))

        })


        // if (element) {
        //     for (var i = 0; i < total_length; i++) {
        //         var $tr = $(element).closest('tr');
        //         var type_id = $tr.data('type_id');

        //         // var $tr = $('.itemTable > tbody > tr').eq(i);
        //         // var group_id = $tr.data('group_id');

        //         // // console.log(group_id ?? "Not added attribute");

        //         // if (group_id !== undefined) {
        //         //     group_id = parseInt(group_id) - 1; // only parse if defined
        //         // } else {
        //         //     group_id = 0; // fallback
        //         // }

        //         var taxable_total = 0;
        //         var qty = $tr.find(".txt_product_qty_" + type_id).val();
        //         if (parseFloat(qty) > 0) {
        //             qty = qty;

        //         } else {
        //             if (type_id == 1 || type_id == 2 || type_id == 3) {
        //                 qty = 1;

        //             } else {
        //                 qty = 0;
        //             }
        //         }

        //         var price = $tr.find(".txt_price_" + type_id).val();
        //         // console.log(`$(".txt_price_" + ${type_id}).eq(${group_id}).val()`);
        //         // console.log("group_id:", '=>', group_id);
        //         // console.log("price:", '=>', price);
        //         if (parseFloat(price) > 0) {
        //             price = price;
        //         } else {
        //             price = 0;
        //         }
        //         taxable_total = parseFloat(qty) * parseFloat(price);
        //         //   $("#txt_total_"+i).val(parseFloat(taxable_total).toFixed(2));
        //         if (parseFloat(taxable_total) > 0) {} else {
        //             rice = 0;
        //         }

        //         taxable_total = parseFloat(qty) * parseFloat(price);
        //         //   $("#txt_total_"+i).val(parseFloat(taxable_total).toFixed(2));

        //         if (parseFloat(taxable_total) > 0) {} else {
        //             total_total_amount = total_total_amount + 0;
        //             // $("#txt_total_"+i).val("");
        //             $tr.find(".txt_final_total_" + type_id).val("");
        //         }

        //         var total_amount = parseFloat(taxable_total);
        //         // console.log("Total Amount:", qty, "*", price, "=", group_id, "=>", total_amount)
        //         total_amounts += taxable_total;
        //         total_total_amount = total_total_amount + total_amount;
        //         // $("#txt_final_total_" + i).val(parseFloat(total_amount).toFixed(2));

        //         // console.log($(`.txt_final_total_${type_id}`).eq(group_id))

        //         $tr.find(`.txt_final_total_${type_id}`).val(parseFloat(total_amount).toFixed(2))

        //     }
        // }
        $("#txt_item_total").val(parseFloat(total_total_amount).toFixed(2));
        var txtstate = $('.txt_state').find("option:selected").attr('data-code');
        console.log(txtstate);
        var csgst_total_final_amount = 0;
        var igst_total_final_amount = 0;
        if (txtstate == 33) {
            /*CGST*/
            var cgst = $("#cgst_per").val();
            if (parseFloat(cgst) > 0) {
                cgst = cgst;
            } else {
                cgst = 0;
            }
            var cgst_per = parseFloat(cgst) / 100;
            var cgst_amt = parseFloat(cgst_per) * (parseFloat(total_amounts));
            if (parseFloat(cgst_amt) > 0) {
                $("#cgst_total").val(parseFloat(cgst_amt).toFixed(2));
                cgst_amttot = cgst_amttot + parseFloat(cgst_amt);
            } else {
                $("#cgst_total").val("");
            }
            /*
                var ctotalamt = 0;
                var cgsttype = $('.txt_intstate').find("option:selected").val();
                if (cgsttype == 1) {
                    var cinclusiveGST = parseFloat(total_total_amount) - parseFloat(cgst_amttot)
                    //$('.txt_total').val(inclusiveGST);
                    $('.txt_item_total').val(cinclusiveGST);
                    ctotalamt = parseFloat(cinclusiveGST);
                } else {
                    var cexclusiveGST = parseFloat(total_total_amount) + parseFloat(cgst_amttot)
                    //$('.txt_total').val(exclusiveGST);
                    $('.txt_item_total').val(total_total_amount);
                    ctotalamt = parseFloat(total_total_amount);
                }
                //  var totalamt = $('.txt_total').val();
                var cgst_total_final_amount = parseFloat(ctotalamt) + parseFloat(cgst_amt);

            */
            /*SGST*/
            var sgst = $("#sgst_per").val();
            if (parseFloat(sgst) > 0) {
                sgst = sgst;
            } else {
                sgst = 0;
            }
            var sgst_per = parseFloat(sgst) / 100;
            var sgst_amt = parseFloat(sgst_per) * (parseFloat(total_amounts));
            if (parseFloat(sgst_amt) > 0) {
                $("#sgst_total").val(parseFloat(sgst_amt).toFixed(2));
                sgst_amttot = sgst_amttot + parseFloat(sgst_amt);
            } else {
                $("#sgst_total").val("");
            }



            var stotalamt = 0;
            var sgsttype = $('.txt_intstate').find("option:selected").val();
            if (sgsttype == 1) {
                var sinclusiveGST = parseFloat(total_total_amount) - (parseFloat(sgst_amttot) + parseFloat(cgst_amttot));
                //$('.txt_total').val(inclusiveGST);
                $('.txt_item_total').val(sinclusiveGST);
                stotalamt = parseFloat(sinclusiveGST);


                var discount_percentage = stotalamt * plan_discount;
                $('#discount_amt').val(discount_percentage.toFixed(2));

                var discount_total = stotalamt - discount_percentage;
                $('#plan_dis_amt').val(discount_total);


            } else {
                var sexclusiveGST = parseFloat(total_total_amount) + (parseFloat(sgst_amttot) + parseFloat(cgst_amttot));
                //$('.txt_total').val(exclusiveGST);
                $('.txt_item_total').val(total_total_amount);
                stotalamt = parseFloat(total_total_amount);

                var discount_percentage = stotalamt * plan_discount;
                $('#discount_amt').val(discount_percentage.toFixed(2));

                var discount_total = stotalamt - discount_percentage;
                $('#plan_dis_amt').val(discount_total);
            }
            //  var totalamt = $('.txt_total').val();
            var csgst_total_final_amount = parseFloat(discount_total) + parseFloat(sgst_amt) + parseFloat(cgst_amt);


        } else {

            /*IGST*/
            var igst = $("#igst_per").val();
            if (parseFloat(igst) > 0) {
                igst = igst;
            } else {
                igst = 0;
            }
            var igst_per = parseFloat(igst) / 100;
            var igst_amt = parseFloat(igst_per) * (parseFloat(total_amounts));
            if (parseFloat(igst_amt) > 0) {
                $("#igst_total").val(parseFloat(igst_amt).toFixed(2));
                igst_amttot = igst_amttot + parseFloat(igst_amt);
            } else {
                $("#igst_total").val("");
            }
            var itotalamt = 0;
            var igsttype = $('.txt_intstate').find("option:selected").val();
            if (igsttype == 1) {
                var iinclusiveGST = parseFloat(total_total_amount) - parseFloat(igst_amttot)
                //$('.txt_total').val(inclusiveGST);
                $('.txt_item_total').val(iinclusiveGST);
                itotalamt = parseFloat(iinclusiveGST);
                var discount_percentage = itotalamt * plan_discount;
                $('#discount_amt').val(discount_percentage.toFixed(2));

                var discount_total = itotalamt - discount_percentage;
                $('#plan_dis_amt').val(discount_total);
            } else {
                var iexclusiveGST = parseFloat(total_total_amount) + parseFloat(igst_amttot)
                //$('.txt_total').val(exclusiveGST);
                $('.txt_item_total').val(total_total_amount);
                itotalamt = parseFloat(total_total_amount);


                var discount_percentage = itotalamt * plan_discount;
                $('#discount_amt').val(discount_percentage.toFixed(2));

                var discount_total = itotalamt - discount_percentage;
                $('#plan_dis_amt').val(discount_total);
            }
            //  var totalamt = $('.txt_total').val();
            var igst_total_final_amount = parseFloat(discount_total) + parseFloat(igst_amt);


        }


        var discount_final1 = $("#discount_final1").val();

        if (parseFloat(discount_final1) > 0) {} else {
            discount_final1 = 0;
        }
        var caltype1 = $("#txt_cal_type1").find("option:selected").val();
        var discount_final_amt1 = '';
        if (caltype1 == 1) {
            $("#discount_final1").removeAttr('max');
            discount_final_amt1 = parseFloat(discount_final1);

        } else {
            $("#discount_final1").attr('max', 100);
            var discount_final_per1 = parseFloat(discount_final1) / 100;
            discount_final_amt1 = parseFloat(discount_final_per1) * parseFloat(total_amounts);

        }


        if (parseFloat(discount_final_amt1) > 0) {
            $("#discount_final_amt1").val(parseFloat(discount_final_amt1).toFixed(2));
        } else {
            $("#discount_final_amt1").val("");
        }
        var discommper1 = 0.00;
        //$('.txt_field_value_comm').each(function() {
        //var calpers=$(this).closest('tr').find('td .txt_cal_types_comm').val();
        var calpers = $('.discount_type1').val();
        //var enteredamt = $(this).val();

        //if(enteredamt){
        if (calpers == 1) {
            discommper1 += parseFloat(discount_final_amt1, 10) || 0;
        }
        if (calpers == 2) {
            discommper1 -= parseFloat(discount_final_amt1, 10) || 0;
        }
        var distot1 = discommper1.toFixed(2);

        //  var distot1 = discommper1.toFixed(2);
        //});



        var discount_final2 = $("#discount_final2").val();

        if (parseFloat(discount_final2) > 0) {} else {
            discount_final2 = 0;
        }


        var caltype2 = $("#txt_cal_type2").find("option:selected").val();
        var discount_final_amt2 = '';
        if (caltype2 == 1) {
            $("#discount_final2").removeAttr('max');
            discount_final_amt2 = parseFloat(discount_final2);

        } else {
            $("#discount_final2").attr('max', 100);

            var discount_final_per2 = parseFloat(discount_final2) / 100;
            discount_final_amt2 = parseFloat(discount_final_per2) * parseFloat(total_amounts);
        }



        if (parseFloat(discount_final_amt2) > 0) {
            $("#discount_final_amt2").val(parseFloat(discount_final_amt2).toFixed(2));
        } else {
            $("#discount_final_amt2").val("");
        }
        //$('.txt_field_value_comm').each(function() {
        //var calpers=$(this).closest('tr').find('td .txt_cal_types_comm').val();
        var calpers = $('.discount_type2').val();
        //var enteredamt = $(this).val();
        var discommper2 = 0.00;

        //if(enteredamt){
        if (calpers == 1) {
            discommper2 += parseFloat(discount_final_amt2, 10) || 0;
        }
        if (calpers == 2) {
            discommper2 -= parseFloat(discount_final_amt2, 10) || 0;
        }

        //});
        var distot2 = discommper2.toFixed(2);

        var discount_final3 = $("#discount_final3").val();

        if (parseFloat(discount_final3) > 0) {} else {
            discount_final3 = 0;
        }


        var caltype3 = $("#txt_cal_type3").find("option:selected").val();
        var discount_final_amt3 = '';
        if (caltype3 == 1) {
            $("#discount_final3").removeAttr('max');
            discount_final_amt3 = parseFloat(discount_final3);

        } else {
            $("#discount_final3").attr('max', 100);
            var discount_final_per3 = parseFloat(discount_final3) / 100;
            discount_final_amt3 = parseFloat(discount_final_per3) * parseFloat(total_amounts);
        }




        if (parseFloat(discount_final_amt3) > 0) {
            $("#discount_final_amt3").val(parseFloat(discount_final_amt3).toFixed(2));
        } else {
            $("#discount_final_amt3").val("");
        }
        //$('.txt_field_value_comm').each(function() {
        //var calpers=$(this).closest('tr').find('td .txt_cal_types_comm').val();
        var calpers3 = $('.discount_type3').val();
        //var enteredamt = $(this).val();
        var discommper3 = 0.00;

        //if(enteredamt){
        if (calpers3 == 1) {
            discommper3 += parseFloat(discount_final_amt3, 10) || 0;
        }
        if (calpers == 2) {
            discommper3 -= parseFloat(discount_final_amt3, 10) || 0;
        }
        var distot3 = discommper3.toFixed(2);

        //  var distot = discommper.toFixed(2);
        //});

        var discount_final4 = $("#discount_final4").val();

        if (parseFloat(discount_final4) > 0) {} else {
            discount_final4 = 0;
        }


        var caltype4 = $("#txt_cal_type4").find("option:selected").val();
        var discount_final_amt4 = '';
        if (caltype4 == 1) {
            $("#discount_final4").removeAttr('max');
            discount_final_amt4 = parseFloat(discount_final4);

        } else {
            $("#discount_final4").attr('max', 100);


            var discount_final_per4 = parseFloat(discount_final4) / 100;
            discount_final_amt4 = parseFloat(discount_final_per4) * parseFloat(total_amounts);
        }


        if (parseFloat(discount_final_amt4) > 0) {
            $("#discount_final_amt4").val(parseFloat(discount_final_amt4).toFixed(2));
        } else {
            $("#discount_final_amt4").val("");
        }
        //$('.txt_field_value_comm').each(function() {
        //var calpers=$(this).closest('tr').find('td .txt_cal_types_comm').val();
        var calpers = $('.discount_type4').val();
        //var enteredamt = $(this).val();
        var discommper4 = 0.00;
        //if(enteredamt){
        if (calpers == 1) {
            discommper4 += parseFloat(discount_final_amt4, 10) || 0;
        }
        if (calpers == 2) {
            discommper4 -= parseFloat(discount_final_amt4, 10) || 0;
        }
        var distot4 = discommper4.toFixed(2);

        // var distot = discommper.toFixed(2);
        //});

        var discount_final5 = $("#discount_final5").val();

        if (parseFloat(discount_final5) > 0) {} else {
            discount_final5 = 0;
        }


        var caltype5 = $("#txt_cal_type5").find("option:selected").val();
        var discount_final_amt5 = '';
        if (caltype5 == 1) {
            $("#discount_final5").removeAttr('max');
            discount_final_amt5 = parseFloat(discount_final5);

        } else {
            $("#discount_final5").attr('max', 100);
            var discount_final_per5 = parseFloat(discount_final5) / 100;
            discount_final_amt5 = parseFloat(discount_final_per5) * parseFloat(total_amounts);
        }


        if (parseFloat(discount_final_amt5) > 0) {
            $("#discount_final_amt5").val(parseFloat(discount_final_amt5).toFixed(2));
        } else {
            $("#discount_final_amt5").val("");
        }
        //$('.txt_field_value_comm').each(function() {
        //var calpers=$(this).closest('tr').find('td .txt_cal_types_comm').val();
        var calpers5 = $('.discount_type5').val();
        //var enteredamt = $(this).val();
        var discommper5 = 0.00;

        //if(enteredamt){
        if (calpers5 == 1) {
            discommper5 += parseFloat(discount_final_amt5, 10) || 0;
        }
        if (calpers == 2) {
            discommper5 -= parseFloat(discount_final_amt5, 10) || 0;
        }

        var distot5 = discommper5.toFixed(2);

        var distotaamount = parseFloat(distot1) + parseFloat(distot2) + parseFloat(distot3);
        //});
        var GST_totamount = parseFloat(csgst_total_final_amount) + parseFloat(igst_total_final_amount);
        var grand_total = parseFloat(GST_totamount) + parseFloat(distotaamount);
        $("#txt_grand_total").val(parseFloat(grand_total).toFixed(2));

        calAdvance();

        // var grand_total = parseFloat(gst_total_final_amount) ;
        //$("#txt_grand_total").val(parseFloat(grand_total).toFixed(2));
    }

    $(document).on("keypress keyup blur", ".numbersOnly", function(event) {
        var $input = $(this);
        var regex = /^\d+$/;
        if (!regex.test($input.val())) {
            $input.val($input.val().replace(/\D/, ''));
        }
    });

    $(document).on("keypress", '.numberss', function(event) {
        if ((event.which != 46 || $(this).val().indexOf('.') != -1) && (event.which < 48 || event.which > 57)) {
            event.preventDefault();
        }
    });

    $('#txt_customer_name').on("change", function(e) {
        var cusId = $(this).find("option:selected").val();
        $.ajax({
            url: "modules/sales/ajax_functions_commercial.php",
            data: {
                'mode': 'getcustomerbasedetails',
                'cusId': cusId,
            },
            type: 'post',
            dataType: 'json',
            success: function(response) {
                //  $('.txt_customer_city ').val(' ');
                $('.txt_customer_city option').attr('selected', false);
                $('.txt_state option').attr('selected', false);


                $('.txt_streetarea').val(' ');
                if (response[0]) {
                    var cus_city = "";
                    if (response[0].cus_city) {
                        cus_city = response[0].cus_city;
                    }
                    var cus_state = "";

                    if (response[0].cus_state) {
                        cus_state = response[0].cus_state;
                    }
                    $('.txt_customer_city ').find('option[value="' + cus_city + '"]').attr("selected",
                        true);
                    $('.txt_state').find('option[value="' + cus_state + '"]').attr("selected", true);

                    $('.txt_streetarea').val(response[0].cus_mob_no);
                    $('#txt_state').trigger('change');

                }
                // $('.txt_state ' + czid).val(' ');
                // $('.txt_price_' + czid).val(response);
                //  cal();
            },
            error: function(response) {
                console.log(response);
            }
        });

    });

    // $.validator.addMethod("noDuplicateMainProduct", function(value, element) {

    //     if (value === '' || value == 0) return true; // ignore empty

    //     var currentRow = $(element).closest('tr')[0];
    //     var count = 0;

    //     // Loop only rows where type_id = 1
    //     $('.itemTable tbody tr[data-type_id="1"] .txt_item').each(function() {

    //         // Skip same row
    //         if ($(this).closest('tr')[0] === currentRow) {
    //             return;
    //         }

    //         if ($(this).val() === value) {
    //             count++;
    //         }
    //     });

    //     return count === 0;

    // }, "This product is already selected in another group");

    $.validator.addMethod("noDuplicateMainProduct", function(value, element) {

        if (!value) return true;

        var $tr = $(element).closest('tr');
        if ($tr.data('type_id') != 1) return true;

        var currentRow = $tr[0];
        var currentGroup = $tr.data('group_id');
        var currentProduct = value;

        var currentCategory = $('tr[data-group_id="' + currentGroup + '"][data-type_id="2"] .txt_item').val() || 0;

        if (currentCategory == 0) return true;

        var duplicateFound = false;

        $('tr[data-type_id="1"]').each(function() {

            var loopRow = $(this);
            var loopGroup = loopRow.data('group_id');

            if (this === currentRow) return;

            var loopProduct = loopRow.find('.txt_item').val();
            var loopCategory = $('tr[data-group_id="' + loopGroup + '"][data-type_id="2"] .txt_item').val() || 0;

            if (loopProduct == currentProduct && loopCategory == currentCategory) {
                duplicateFound = true;
                return false;
            }
        });

        return !duplicateFound;

    }, "This Product + Category combination already exists in another group");



    $(document).on('change', '.txt_item', function() {

        // Revalidate all product fields
        $('.itemTable tbody tr[data-type_id="1"] .txt_item').each(function() {
            $(this).valid();
        });
    });



    // Initialize the form validation
    $('#form_estimate_noncomm_add').validate({
        rules: {
            txt_customer_name: {
                required: true
            },
            txt_pi_date: {
                required: true
            },
            txt_customer_city: {
                required: true
            },
            txt_state: {
                required: true
            },
            txt_franchise: {
                required: true
            },

            // 'txt_item[]': {
            //     required: function(element) {
            //         var $tr = $(element).closest("tr");
            //         // var ridx = $tr.index();
            //         // console.log("index =>", ridx);
            //         console.log(element)

            //         var czid = $(element).attr('data-czid');
            //         var itemType = $tr.find('.txt_itemtypes_' + czid).val() ?? 0;
            //         var selectedItem = $tr.find('.txt_item_' + czid).val() ?? 0;
            //         var quantity = $tr.find('.txt_product_qty_' + czid).val() ?? 0;
            //         var price = $tr.find('.txt_price_' + czid).val() ?? 0;
            //         // var finalPrice = $tr.find('.txt_final_total_' + czid);
            //         // var grandTotal = $tr.find('.txt_item_total_' + czid);

            //         if (czid == 3) {
            //             console.log("itemType : ", itemType, `$(element).closest("tr").find('.txt_itemtypes_' + ${czid}).val()`);
            //             console.log("selectedItem : ", selectedItem);
            //             console.log("quantity : ", quantity);
            //             console.log("price : ", price);
            //             // console.log("finalPrice : ", finalPrice);
            //             // console.log("grandTotal : ", grandTotal);
            //         }

            //         var quantityValue = parseInt(quantity, 10);
            //         var priceValue = parseFloat(price);

            //         if (czid == 3) {
            //             console.log(`(${itemType} == 0 || (${itemType} >= '1' && ${itemType} <= '8')) && ${selectedItem} == 0 && (
            //                 ${quantityValue} == 0 && ${priceValue} == 0)`);
            //         }

            //         // if ((itemType == 0 || (itemType >= '1' && itemType <= '8')) && selectedItem == 0 && (
            //         //         quantityValue == 0 && priceValue == 0)) {
            //         //     console.log('false')
            //         //     return false;
            //         // }

            //         return true;
            //     }
            // },
            'txt_item[]': {
                required: function(element) {
                    var $tr = $(element).closest("tr");
                    var czid = $(element).attr('data-czid');
                    var itemType = $tr.find('.txt_itemtypes').val();
                    var selectedItem = $tr.find('.txt_item').val();
                    var quantityField = $tr.find('.txt_product_qty');
                    var priceField = $tr.find('.txt_price');

                    var quantityValue = parseInt(quantityField.val(), 10);
                    var priceValue = parseFloat(priceField.val());

                    // if (czid == 1) {
                    //     console.log(`(${itemType} === '0' || (${itemType} >= '1' && ${itemType} <= '8')) && ${selectedItem} === '' && (
                    //         ${quantityValue} === 0 && ${priceValue} === 0)`)
                    // }

                    if ((itemType === '0' || (itemType >= '1' && itemType <= '8')) && selectedItem === '' && (
                            quantityValue === 0 && priceValue === 0)) {
                        return false;
                    }

                    return true;
                },
                noDuplicateMainProduct: true
            },
            // 'txt_item[]': {
            //     required: function(element) {
            //         var czid = $(element).attr('data-czid');
            //         var itemType = $('#txt_itemtypes_' + czid).val();
            //         var selectedItem = $('#txt_item_' + czid).val();
            //         var quantityField = $('#txt_product_qty_' + czid);
            //         var priceField = $('#txt_price_' + czid);
            //         var finalPrice = $('#txt_final_total_' + czid);
            //         var grandTotal = $('#txt_item_total_' + czid);

            //         var quantityValue = parseInt(quantityField.val(), 10);
            //         var priceValue = parseFloat(priceField.val());

            //         // if (itemType >= '5' && itemType <= '8' && selectedItem !== '') {
            //         //     quantityField.val(8).trigger('change');
            //         // } else {
            //         //     quantityField.val(0).trigger('change');
            //         // }


            //         // if (czid == 1) {
            //         //     console.log(`(${itemType} === '0' || (${itemType} >= '1' && ${itemType} <= '8')) && ${selectedItem} === '' && (
            //         //                 ${quantityValue} === 0 && ${priceValue} === 0)`)
            //         // }

            //         if ((itemType === '0' || (itemType >= '1' && itemType <= '8')) && selectedItem === '' && (
            //                 quantityValue === 0 && priceValue === 0)) {
            //             return false;
            //         }

            //         return true;
            //     }
            // },

            'txt_types[]': {
                required: true
            },

            'txt_price_type[]': {
                required: function(element) {
                    var $tr = $(element).closest("tr");
                    var categoryId = $(element).attr('id').replace('txt_price_type_', '');
                    var itemType = $tr.find('.txt_itemtypes').val();
                    var selectedItem = $tr.find('.txt_item').val();
                    if (itemType == '2' && selectedItem !== " ") {
                        return true;
                    }
                    return false;
                }
            },

            'txt_product_qty[]': {
                required: true,
                // min: function(element) {
                //     var $tr = $(element).closest("tr");
                //     // console.log("element=>", element)
                //     // console.log("element ID=>", $(element).attr('id'))
                //     var czid = $(element).attr('id').replace('txt_product_qty_', '');
                //     var itemType = $tr.find('.txt_itemtypes').val();
                //     var selectedItem = $tr.find('.txt_item').val();
                //     // console.log("selectedItem=>", selectedItem)
                //     if (selectedItem !== "" && (itemType === '2' || itemType === '5' || itemType === '6' ||
                //             itemType === '8')) {
                //         return 1;
                //     } else {
                //         return 0;
                //     }
                // }
                min: function(element) {
                    var $tr = $(element).closest("tr");
                    var selectedItem = $tr.find('.txt_item').val();
                    var itemType = $tr.find('.txt_itemtypes').val();
                    if (selectedItem !== "" && selectedItem != 0 && ['1', '2', '3', '4', '5', '6', '7', '8'].includes(itemType)) {
                        return 1;
                    } else {
                        return 0;
                    }
                }
            },
            'txt_price[]': {
                required: function(element) {
                    var $tr = $(element).closest("tr");
                    var czid = $(element).attr('id').replace('txt_price_', '');
                    var itemType = $tr.find('.txt_itemtypes').val();
                    var selectedItem = $tr.find('.txt_item').val();

                    return (itemType === '2' || itemType === '5' || itemType === '6' ||
                            itemType === '8') &&
                        selectedItem !== '';
                },
                min: function(element) {
                    var $tr = $(element).closest("tr");
                    var czid = $(element).attr('id').replace('txt_price_', '');
                    var itemType = $tr.find('.txt_itemtypes').val();
                    var selectedItem = $tr.find('.txt_item').val();

                    // Check if it's item type 5 to 8 and the selected price is greater than 0
                    if ((itemType === '2' || itemType === '5' || itemType === '6' ||
                            itemType === '8') &&
                        parseFloat(selectedItem) > 0) {
                        return true;
                    }
                    return false;
                }
            },

            'txt_final_total[]': {
                required: true
            },
            txt_item_total: {
                required: true,
            },
            txt_grand_total: {
                required: true
            },
            txt_streetarea: {
                required: true
            },
            discount_field1: {
                required: true
            },
            discount_final1: {
                required: function(element) {
                    var discountField1Value = $('#discount_field1').val();
                    return (discountField1Value === '1' || discountField1Value === '2' || discountField1Value === '3');
                }
            },
        },
        messages: {
            txt_customer_name: {
                required: 'This field is required.'
            },
            txt_pi_date: {
                required: 'This field is required.'
            },
            txt_franchise: {
                required: 'This field is required.'
            },
            'txt_types[]': {
                required: 'This field is required.'
            },
            'txt_item[]': {
                required: 'This field is required.'
            },

            'txt_price_type[]': {
                required: 'This field is required.'
            },
            'txt_product_qty[]': {
                required: 'This field is required.',
                min: 'Quantity must be at least 1.',
                checkZeroQuantity: 'Quantity cannot be 0.'
            },
            'txt_price[]': {
                required: 'This field is required.',
                min: 'Price must be greater than 0.'
            },
            'txt_final_total[]': {
                required: 'This field is required.'
            },
            txt_item_total: {
                required: 'This field is required.'

            },
            txt_grand_total: {
                required: 'This field is required.'
            },
            txt_streetarea: {
                required: 'This field is required.'
            },
            discount_field1: {
                required: 'Please select.'
            },
            discount_final1: {
                required: 'Please enter price.'
            },

        },
        checkZeroQuantity: function(value, element) {
            var itemType = $(element).closest('tr').find('.txt_itemtypes').val();
            if (['5', '6', '7', '8'].includes(itemType) && value === '0') {
                return false; // Quantity cannot be 0 for these item types
            }
            return true; // Validation passes
        },
        errorPlacement: function(error, element) {
            if (element.attr("name") === 'txt_customer_name') {
                error.insertAfter($("#txt_customer_name-error"));
            } else {
                error.insertAfter(element);
            }
        },
        submitHandler: function(form) {
            var formData = new FormData($('#form_estimate_noncomm_add')[0]);
            // for (const pair of formData.entries()) {
            //     console.log(pair[0], pair[1]);
            // }
            // return;
            $('.btn_save').prop("disabled", true);

            $.ajax({
                url: "modules/sales/ajax_functions_noncommercial.php",
                data: formData,
                type: 'post',
                async: false,
                dataType: 'json',
                // success: function(response) {
                //     if (response) {
                //         swal({
                //             title: "Success!",
                //             text: "Sales Order has been added successfully!.",
                //             type: "success",
                //             timer: 1000,
                //             buttons: false,
                //         }).then(function() {
                //             window.location.href = "index.php?erp=69&id=" + response;
                //         });
                //     } else {
                //         swal("Failed!", "Something went wrong, Try again!", "error");
                //     }
                // },
                success: function(response) {
                    if (response) {

                        // startRazorpayPayment(response);
                        window.open(response.whatsapp, "_blank")

                    } else {
                        swal("Failed!", "Something went wrong!", "error");
                    }
                },
                error: function(response) {
                    $('.btn_save').prop("disabled", false);
                    console.log(response);
                    swal("Error!", "An error occurred while processing your request.", "error");
                },
                cache: false,
                contentType: false,
                processData: false
            });
        }
    });

    // $(document).ready(function() {
    //     // Add an event listener to the item selection fields
    //     $('select[id^="txt_item_"]').on('change', function() {
    //         var czid = $(this).attr('id').replace('txt_item_', '');
    //         var quantityField = $('#txt_product_qty_' + czid);
    //         var priceField = $('#txt_price_' + czid);
    //         var finalPrice = $('#txt_final_total_' + czid);

    //         var selectedItem = $(this).val();

    //         if (selectedItem === '') {
    //             // If item is empty, set quantity, price, and final total to 0
    //             quantityField.val(0).trigger('input');
    //             priceField.val(0).trigger('input');
    //             finalPrice.val('0.00').trigger('input');
    //         }
    //     });

    //     // Custom validation for the 'txt_item[]' fields
    //     $.validator.addMethod("customItemValidation", function(value, element) {
    //         var czid = $(element).attr('data-czid');
    //         var itemType = $('#txt_itemtypes_' + czid).val();
    //         var selectedItem = $('#txt_item_' + czid).val();
    //         var quantityField = $('#txt_product_qty_' + czid);
    //         var priceField = $('#txt_price_' + czid);

    //         var quantityValue = parseInt(quantityField.val(), 10);
    //         var priceValue = parseFloat(priceField.val());

    //         // Don't show validation message for 'txt_item[]' if item type is '2' or between '5' and '8' and selected item is empty
    //         if ((itemType === '0' || (itemType >= '1' && itemType <= '8')) && selectedItem === '') {
    //             return true;
    //         }

    //         // Show validation message for 'txt_item[]' if item is empty and either quantity or price is manually typed as '0'
    //         return (selectedItem !== '' || (quantityValue !== 0 && !isNaN(quantityValue)) || (priceValue !==
    //             0 && !isNaN(priceValue)));
    //     }, "Please select an item or ensure quantity or price are not manually typed as '0'.");


    //     $('#form_estimate_noncomm_add').validate({
    //         rules: {
    //             txt_customer_name: {
    //                 required: true
    //             },
    //             txt_pi_date: {
    //                 required: true
    //             },
    //             txt_customer_city: {
    //                 required: true
    //             },
    //             txt_state: {
    //                 required: true
    //             },
    //             txt_franchise: {
    //                 required: true
    //             },

    //             // 'txt_item[]': {

    //             //     required: function(element) {
    //             //         var czid = $(element).attr('data-czid');
    //             //         var itemType = $('#txt_itemtypes_' + czid).val();
    //             //         var selectedItem = $('#txt_item_'+ czid).val();

    //             //         if (selectedItem === '' && (itemType === '2' || (itemType >= '5' && itemType <=
    //             //                 '8'))) {
    //             //             var quantityField = $('#txt_product_qty_' + czid);
    //             //             var priceField = $('#txt_price_' + czid);
    //             //             var quantityValue = parseInt(quantityField.val(), 10);
    //             //             var priceValue = parseFloat(priceField.val());
    //             //             quantityField.val(0);
    //             //             priceField.val(0);
    //             //         }
    //             //         else {

    //             //             $('#txt_product_qty_' + czid).val(1);
    //             //             //$('#txt_price_' + czid).val(1);

    //             //         }


    //             //     }
    //             // },



    //             // 'txt_item[]': {
    //             //     required: function(element) {
    //             //         var czid = $(element).attr('data-czid');
    //             //         var itemType = $('#txt_itemtypes_' + czid).val();
    //             //         var selectedItem = $('#txt_item_' + czid).val();
    //             //         var quantityField = $('#txt_product_qty_' + czid);
    //             //         var priceField = $('#txt_price_' + czid);
    //             //         var finalPrice = $('#txt_final_total_' + czid);                
    //             //         var quantityValue = parseInt(quantityField.val(), 10);
    //             //         var priceValue = parseFloat(priceField.val());

    //             //         if ((itemType === '2' || (itemType >= '5' && itemType <= '8')) && selectedItem === '') {
    //             //             // If item type is 2 or between 5 and 8 and item is empty, set quantity and price to 0
    //             //             quantityField.val(0);
    //             //             priceField.val(0);
    //             //             finalPrice.val(0.00);
    //             //         }

    //             //         if (quantityValue !== 0 || priceValue !== 0) {
    //             //             // If quantity or price is not equal to 0, consider txt_item[] as required
    //             //             return true;
    //             //         } else {
    //             //             return false; // Consider txt_item[] as not required
    //             //         }
    //             //     }
    //             // },

    //             // 'txt_item[]': {
    //             //     required: function(element) {
    //             //         var czid = $(element).attr('data-czid');
    //             //         var itemType = $('#txt_itemtypes_' + czid).val();
    //             //         var selectedItem = $('#txt_item_' + czid).val();
    //             //         var quantityField = $('#txt_product_qty_' + czid);
    //             //         var priceField = $('#txt_price_' + czid);
    //             //         var finalPrice = $('#txt_final_total_' + czid); 
    //             //         var quantityValue = parseInt(quantityField.val(), 10);
    //             //         var priceValue = parseFloat(priceField.val());

    //             //         if ((itemType === '2' || (itemType >= '5' && itemType <= '8')) && selectedItem === '') {
    //             //             // If item type is 2 or between 5 and 8 and item is empty, set quantity, price, and final total to 0
    //             //             quantityField.val(0).trigger('change');
    //             //             priceField.val(0).trigger('change');
    //             //             finalPrice.val('0.00').trigger('change');
    //             //             return false;
    //             //         }

    //             //         // Show txt_item[] validation error only when both quantity, price, and final total are not 0
    //             //         if (quantityValue !== 0 || priceValue !== 0 || finalPrice.val() !== '0.00') {
    //             //             return true; // Consider txt_item[] as required
    //             //         } else {
    //             //             return false; // Don't consider txt_item[] as required
    //             //         }
    //             //     }
    //             // },

    //             'txt_item[]': {
    //                 customItemValidation: true
    //             },



    //             'txt_types[]': {
    //                 required: true
    //             },
    //             'txt_price_type[]': {
    //                 required: function(element) {
    //                     var categoryId = $(element).attr('id').replace('txt_price_type_', '');
    //                     var itemType = $('#txt_itemtypes_' + categoryId).val();
    //                     var selectedItem = $('#txt_item_' + categoryId).val();
    //                     if (itemType == '2' && selectedItem !== 'SELECT ONE') {
    //                         return true;
    //                     }
    //                     return false;
    //                 }
    //             },

    //             'txt_product_qty[]': {
    //                 required: true,
    //                 min: function(element) {
    //                     var czid = $(element).attr('id').replace('txt_product_qty_', '');
    //                     var itemType = $('#txt_itemtypes_' + czid).val();
    //                     var selectedItem = $('#txt_item_' + czid).val();

    //                     // Check if the selected item is not empty and falls into item types 5 to 8
    //                     if (selectedItem !== "" && (itemType === '5' || itemType === '6' || itemType ===
    //                             '7' ||
    //                             itemType === '8')) {
    //                         return 1; // Minimum quantity of 1 is required
    //                     } else {
    //                         return 0; // No minimum validation if the item is empty or not in item types 5 to 8
    //                     }
    //                 }
    //             },


    //             // 'txt_product_qty[]': {
    //             //     required: function(element) {
    //             //         var czid = $(element).attr('id').replace('txt_product_qty_', '');
    //             //         var selectedItem = $('#txt_item_' + czid).val();

    //             //         if (selectedItem === '') {
    //             //             $('#txt_product_qty_' + czid).val(0);
    //             //             return false;
    //             //         } else {
    //             //             return true;
    //             //         }
    //             //     },
    //             //     min: function(element) {
    //             //         var czid = $(element).attr('id').replace('txt_product_qty_', '');
    //             //         var selectedItem = $('#txt_item_' + czid).val();

    //             //         if (selectedItem === '') {
    //             //             return 0;
    //             //         } else {
    //             //             return 1;
    //             //         }
    //             //     }
    //             // },

    //             'txt_price[]': {
    //                 required: function(element) {
    //                     var czid = $(element).attr('id').replace('txt_price_', '');
    //                     var itemType = $('#txt_itemtypes_' + czid).val();
    //                     var selectedItem = $('#txt_item_' + czid).val();

    //                     return (itemType === '2' || itemType === '5' || itemType === '6' || itemType ===
    //                             '7' ||
    //                             itemType === '8') &&
    //                         selectedItem !== '';
    //                 },
    //                 min: function(element) {
    //                     var czid = $(element).attr('id').replace('txt_price_', '');
    //                     var itemType = $('#txt_itemtypes_' + czid).val();
    //                     var selectedItem = $('#txt_item_' + czid).val();

    //                     // Check if it's item type 5 to 8 and the selected price is greater than 0
    //                     if ((itemType === '2' || itemType === '5' || itemType === '6' || itemType ===
    //                             '7' ||
    //                             itemType === '8') &&
    //                         parseFloat(selectedItem) > 0) {
    //                         return true;
    //                     }
    //                     return false;
    //                 }
    //             },



    //             'txt_final_total[]': {
    //                 required: true
    //             },
    //             txt_item_total: {
    //                 required: true,
    //             },
    //             txt_grand_total: {
    //                 required: true
    //             },
    //             txt_streetarea: {
    //                 required: true
    //             },
    //             discount_field1: {
    //                 required: true
    //             },
    //             discount_final1: {
    //                 required: function(element) {
    //                     var discountField1Value = $('#discount_field1').val();
    //                     return (discountField1Value === '1' || discountField1Value === '2');
    //                 }
    //             },
    //         },
    //         messages: {
    //             txt_customer_name: {
    //                 required: 'This field is required.'
    //             },
    //             txt_pi_date: {
    //                 required: 'This field is required.'
    //             },
    //             txt_franchise: {
    //                 required: 'This field is required.'
    //             },
    //             'txt_types[]': {
    //                 required: 'This field is required.'
    //             },
    //             'txt_item[]': {
    //                 required: 'This field is required.'
    //             },

    //             'txt_price_type[]': {
    //                 required: 'This field is required.'
    //             },
    //             'txt_product_qty[]': {
    //                 required: 'This field is required.',
    //                 min: 'Quantity must be at least 1.',
    //                 checkZeroQuantity: 'Quantity cannot be 0.'
    //             },
    //             'txt_price[]': {
    //                 required: 'This field is required.',
    //                 min: 'Price must be greater than 0.'
    //             },
    //             'txt_final_total[]': {
    //                 required: 'This field is required.'
    //             },
    //             txt_item_total: {
    //                 required: 'This field is required.'

    //             },
    //             txt_grand_total: {
    //                 required: 'This field is required.'
    //             },
    //             txt_streetarea: {
    //                 required: 'This field is required.'
    //             },
    //             discount_field1: {
    //                 required: 'Please select.'
    //             },
    //             discount_final1: {
    //                 required: 'Please enter price.'
    //             },

    //         },
    //         checkZeroQuantity: function(value, element) {
    //             var itemType = $(element).closest('tr').find('.txt_itemtypes').val();
    //             if (['5', '6', '7', '8'].includes(itemType) && value === '0') {
    //                 return false; // Quantity cannot be 0 for these item types
    //             }
    //             return true; // Validation passes
    //         },
    //         errorPlacement: function(error, element) {
    //             if (element.attr("name") === 'txt_customer_name') {
    //                 error.insertAfter($("#txt_customer_name-error"));
    //             } else {
    //                 error.insertAfter(element);
    //             }
    //         },
    //         submitHandler: function(form) {
    //             var formData = new FormData($('#form_estimate_noncomm_add')[0]);
    //             $('.btn_save').prop("disabled", true);

    //             $.ajax({
    //                 url: "modules/sales/ajax_functions_noncommercial.php",
    //                 data: formData,
    //                 type: 'post',
    //                 async: false,
    //                 dataType: 'json',
    //                 success: function(response) {
    //                     if (response) {
    //                         swal({
    //                             title: "Success!",
    //                             text: "Sales Order has been added successfully!.",
    //                             type: "success",
    //                             timer: 1000,
    //                             buttons: false,
    //                         }).then(function() {
    //                             window.location.href = "index.php?erp=69&id=" +
    //                                 response;
    //                         });
    //                     } else {
    //                         swal("Failed!", "Something went wrong, Try again!", "error");
    //                     }
    //                 },
    //                 error: function(response) {
    //                     $('.btn_save').prop("disabled", false);
    //                     console.log(response);
    //                     swal("Error!", "An error occurred while processing your request.",
    //                         "error");
    //                 },
    //                 cache: false,
    //                 contentType: false,
    //                 processData: false
    //             });
    //         }
    //     });

    // });



    $("input:checkbox").on('click', function() {
        // in the handler, 'this' refers to the box clicked on
        var $box = $(this);
        if ($box.is(":checked")) {
            var group = "input:checkbox[name='" + $box.attr("name") + "']";
            $(group).prop("checked", false);
            $box.prop("checked", true);
        } else {
            $box.prop("checked", false);
        }

        function enableFields(czid) {
            var itemSelect = document.getElementById('txt_item_' + czid);
            var quantityInput = document.getElementById('txt_product_qty_' + czid);
            var categorySelect = document.getElementById('txt_category_' + czid);

            // Set initial value of quantity field to 0
            quantityInput.value = '0';

            itemSelect.addEventListener('change', function() {
                updateQuantity();
            });

            categorySelect.addEventListener('change', function() {
                updateQuantity();
            });

            function updateQuantity() {
                if (itemSelect.value !== 'SELECT ONE' && categorySelect.value !== 'SELECT ONE') {
                    quantityInput.value = '0';
                } else {
                    quantityInput.value = '1';
                }
            }
        }

        <?php for ($j = 1; $j <= $i; $j++) { ?>
            enableFields(<?php echo $j; ?>);
        <?php } ?>
    });


    //fetching a customer plan
    function triggerCustomer(pk_cus_id) {
        if (!pk_cus_id) {
            // Clear the label and reset the discount if no customer is selected
            document.querySelector('label[for="customerPlanLabel"] strong').textContent = "Customer Plan: Not Selected (0%)";
            document.getElementById('plan_discount').value = '0.0';
            document.querySelector('input[name="plan_dis_amt"]').value = '0.0';
            cal(); // Recalculate totals with zero discount
            return;
        }

        $.ajax({
            url: 'modules/sales/add_plan_ajax.php',
            type: 'POST',
            data: {
                pk_cus_id: pk_cus_id
            },
            success: function(response) {
                try {
                    // Parse the JSON response
                    const parsedResponse = JSON.parse(response);

                    // Check if response status is success and data is an array
                    if (parsedResponse.status === "success" && Array.isArray(parsedResponse.data)) {
                        // Find the plan for the selected customer
                        const selectedPlan = parsedResponse.data.find(plan => plan.pk_cus_id == pk_cus_id);

                        if (selectedPlan && selectedPlan.choosenplan) {
                            // Update the label and discount
                            document.querySelector('label[for="customerPlanLabel"] strong').textContent = `Customer Plan: ${selectedPlan.choosenplan} - ${selectedPlan.discount}%`;
                            document.getElementById('plan_discount').value = (selectedPlan.discount) / 100;

                            // Trigger recalculation of the discount amount
                            cal();
                        } else {
                            // If no plan found for the customer
                            document.querySelector('label[for="customerPlanLabel"] strong').textContent = "Customer Plan: Not Found (0%)";
                            document.getElementById('plan_discount').value = "0";
                            cal();
                        }
                    } else {
                        console.error("Invalid response format:", response);
                        document.querySelector('label[for="customerPlanLabel"] strong').textContent = "Customer Plan: Not Found (0%)";
                        document.getElementById('plan_discount').value = "0";
                        cal();
                    }

                } catch (error) {
                    console.error("Error parsing response:", error);
                    document.querySelector('label[for="customerPlanLabel"] strong').textContent = "Customer Plan: Parsing Error";
                    document.getElementById('plan_discount').value = "0";
                    cal();
                }

            },
            error: function(xhr, status, error) {
                console.error("AJAX error:", error);
                document.querySelector('label[for="customerPlanLabel"] strong').textContent = "Customer Plan: AJAX Error";
                document.getElementById('plan_discount').value = "0";
                cal();
            }
        });
    }


    function customerChange() {
        const pk_cus_id = $("#txt_customer_name").val();
        console.log("Selected Customer ID:", pk_cus_id);
        triggerCustomer(pk_cus_id); // Trigger customer plan logic
    }
</script>

<script>
    $(document).ready(function() {

        $('.calendar-row').each(function() {
            let czid = $(this).find('.txt_item').data('czid');

            loadCalendarCategory(czid);
            loadCalendarSize(czid);
            loadCalendarInnerSheet(czid);
            loadCalendarSpecialEffect(czid);
        });

    });

    function loadCalendarCategory(czid) {
        $.post("modules/sales/ajax_functions_noncommercial.php", {
                mode: "getCalendarCategory"
            },
            function(res) {
                let opt = '<option value="">Select Category</option>';
                res.forEach(v => opt += `<option value="${v.id}">${v.name}</option>`);
                $('.txt_category_' + czid).html(opt);
            }, 'json');
    }

    function loadCalendarSize(czid) {
        $.post("modules/sales/ajax_functions_noncommercial.php", {
                mode: "getCalendarSize"
            },
            function(res) {
                let opt = '<option value="">Select Size</option>';
                res.forEach(v => opt += `<option value="${v.id}">${v.size}</option>`);
                $('.txt_size_' + czid).html(opt);
            }, 'json');
    }

    function loadCalendarInnerSheet(czid) {
        $.post("modules/sales/ajax_functions_noncommercial.php", {
                mode: "getCalendarInnerSheet"
            },
            function(res) {
                let opt = '<option value="">Select Inner Sheet</option>';
                res.forEach(v => opt += `<option value="${v.id}">${v.name}</option>`);
                $('.txt_innersheet_' + czid).html(opt);
            }, 'json');
    }

    function loadCalendarSpecialEffect(czid) {
        $.post("modules/sales/ajax_functions_noncommercial.php", {
                mode: "getCalendarSpecialEffect"
            },
            function(res) {
                let opt = '<option value="">Select Effect</option>';
                res.forEach(v => opt += `<option value="${v.id}">${v.name}</option>`);
                $('.txt_specialeffects_' + czid).html(opt);
            }, 'json');
    }
</script>
<script>
    $(document).ready(function() {

        $('.calendar-row .txt_item').each(function() {
            var czid = $(this).data('czid');

            $.ajax({
                url: "modules/sales/ajax_functions_noncommercial.php",
                type: "POST",
                dataType: "json",
                data: {
                    mode: "getCalendarItems"
                },
                success: function(response) {
                    var sel = $('.txt_item_' + czid);
                    sel.html('<option value="">SELECT ONE</option>');

                    $.each(response, function(i, obj) {
                        sel.append('<option value="' + obj.id + '">' + obj.name + '</option>');
                    });
                }
            });
        });

    });
</script>

<script src="https://checkout.razorpay.com/v1/checkout.js"></script>

<script>
    function startRazorpayPayment(data) {

        var options = {
            key: "rzp_test_SSxGqhTR1IzZdx",
            amount: data.amount * 100,
            currency: "INR",
            name: "Your Company",
            description: "Order Payment",
            order_id: data.razorpay_order_id, 

            handler: function(response) {

                $.post("verify_payment.php", {
                    razorpay_order_id: data.razorpay_order_id,
                    razorpay_payment_id: response.razorpay_payment_id,
                    razorpay_signature: response.razorpay_signature,
                    soid: data.soid
                }, function(res) {

                    if (res.status == "success") {
                        alert("Payment Successful");
                        window.location.href = "success.php?id=" + data.soid;
                    } else {
                        alert("Payment verification failed");
                        $('.btn_save').prop("disabled", false);
                    }

                }, 'json');
            },

            modal: {
                ondismiss: function() {
                    alert("Payment popup closed");
                    $('.btn_save').prop("disabled", false);
                }
            }
        };

        var rzp = new Razorpay(options);
        rzp.open();
    }
</script>