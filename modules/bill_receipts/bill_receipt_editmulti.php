    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
				<?php  if($_GET['type']==1){ ?>
                    <h1>Edit Bill Receipt Commercial</h1>
				<?php } elseif($_GET['type']==2){ ?>
				 <h1>Edit Bill Receipt NonCommercial</h1>
				<?php } ?>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="index.php">Home</a></li>
                        <li class="breadcrumb-item active">Edit Bill Receipt</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
...
?>
					...
					<div class="card-footer text-right">
								...
								<?php if($_GET['type']==1){ ?>
                                <input type="hidden" name="mode" value="editBillcommmulti" />
								<?php } elseif($_GET['type']==2){ ?>
								 <input type="hidden" name="mode" value="editBillNonCommmulti" />
								   <?php } ?>
...
function getSOEditValues(soid) {
	<?php if($_GET['type']==1){ ?>
        ...
        data: {
            'soid': soid,
            'mode': 'getBillEditValues_commmulti'
        },
        type: 'post',
        dataType: 'json',
        beforeSend: function() {
        ...
        success: function(response) {
            if (soid == 0) {
				...
				//$('.txt_customer_name').find('option[value="' + response[0].customer_id + '"]').attr("selected", true);
                    $('.txt_customer_name').find('option[value="' + response[0].customer_id + '"]').trigger('chnage');
	...
	<?php }
        ...
        data: {
            'soid': soid,
            'mode': 'getBillEditValues_noncommmulti'
        },
        type: 'post',
        dataType: 'json',
        beforeSend: function() {
    ...
    rules: {
		...
		adv_amount:{
			...
			remote: {
                        ...
                        type: "post",
                        data: {
							mode: "check_amount_bill_by_esidmulti",
                            adv_amount: function() {
