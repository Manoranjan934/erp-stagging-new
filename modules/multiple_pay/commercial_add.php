                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-primary">
                        ...
                        <form class="form-horizontal theme-form" id="form_advance_comm_add" autocomplete="off"
                                                    ...
                                                    <select class="form-control txt_customer_name "
                                                        ...
                                                        <?php  for ($i = 0; $i < count($cus_data); $i++) {
                                                           ...
                                                           if($estimatedata['customer_id']  == $cus_data[$i]['pk_cus_id']){
                                                            ...
                                                            ?>
                                                        <option <?php echo $selcurrem; ?> value="<?php echo $cus_data[$i]['pk_cus_id']; ?>">
                            ...
                            <span class="col-md-2">   <input type="hidden" name="deleted" value="0" id="deleted" />
								<?php if($_GET['type']==1){ ?>
...
/*$(function() {
	//alert('hlooooooooo');
    //$('.txt_customer_name').find('option[value=27]').attr("selected", true);
	//$('.txt_customer_name').find('option[value="27"]').attr("selected", true);
	$('.txt_customer_name').val("27");
	$("#txt_customer_name option[value='29']").prop('selected', true);
});
 ...
 function getCustomerBaseEstimate() {
	 //alert($('option:selected', this).val());
        ...
        success: function(response) {
            ...
            setTimeout(function() {
                $('#txt_estimate_name').find('option[value="'+ <?php echo $_GET['id']; ?> +'"]').attr("selected", true);
            }, 1000);
            }
/*
         //  if(response.length){
	...
	$("#txt_estimateere_name").change(function() {
	 ...
	 //var cus = $("#txt_customer_name").val());
        ...
        success: function(response) {
			...
			for(var i=0;i<=response.length;i++){
				$('#txt_customer_name').append($('<option value="'+response[i].customer_id+'">'+response[i].cus_name+'</option>'));
			}
            ...
            success: function(response) {
                if (response) {
                    ...
                    swal({
                        ...
                        window.location.href = "index.php?erp=83&type=1";
                    });
					} else {
            ...
            success: function(response) {
                if (response) {
                    ...
                    swal({
                        ...
                        window.location.href = "index.php?erp=83&type=2";
                    });
					} else {
                    swal("Failed!", "Something went wrong, Try again!", "error");
                }
