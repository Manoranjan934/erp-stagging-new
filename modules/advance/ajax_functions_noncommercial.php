...
if (isset($_POST['mode']) && $_POST['mode'] == 'addAdvanceNonComm') {
	...
	$result = $obj_advancecommornon->add_advance_noncomm($ad_date);
	echo json_encode($result);
}
if (isset($_POST['mode']) && $_POST['mode'] == 'editAdvanceNonComm') {
	$txt_pi_date = str_replace('/', '-', $_POST['txt_pi_date']);
	...
	$result = $obj_advancecommornon->edit_advance_noncomm($ad_date);
	echo json_encode($result);
}
...
if (isset($_POST['mode']) && $_POST['mode'] == 'updateEstimateNonComm') {
	...
	$obj_advancecommornon->setshipment_to(date('Y-m-d', strtotime($txt_delivery_date)));
	$obj_advancecommornon->setreference_number($_POST['txt_state']);
	$obj_advancecommornon->setproduct_id($_POST['txt_franchise']);
	...
	$soid = $obj_advancecommornon->updateEstimateNonComm($_POST['txt_so_id']);
	$obj_advancecommornon->deleteEstimateNonCommProduct($_POST['txt_so_id']);
	...
	for ($i = 0; $i < count($_POST['txt_itemtypes']); $i++) {
		...
		$obj_advancecommornon->setprice($_POST['txt_price'][$i]);
		$obj_advancecommornon->setfinal_total($_POST['txt_final_total'][$i]);
		$status = $obj_advancecommornon->addEstimateNonCommProduct();
	}
	if ($status) {
...
if (isset($_POST['mode']) && $_POST['mode'] == 'listAdvanceNonComm') {

	$obj_advancecommornon->listAdvanceNonComm();
}

if (isset($_POST['mode']) && $_POST['mode'] == 'getAdvanceEditValues') {
	$id = $_POST['soid'];

	$get_data = $obj_advancecommornon->get_edit_advance_details_non_comm($id);
	$advance = array();
...
if (isset($_POST['mode']) && $_POST['mode'] == 'getSOEditValues') {
	...
	$getSQId = $obj_advancecommornon->getEstimateNonCommById($_POST['soid']);

	$datas = mysqli_fetch_array($getSQId);

	//	$obj_obj_estimatecommornonso->setSQId($_POST['soid']);
	$scproduct = array();
	$getSQProducts = $obj_advancecommornon->getEstimateNonCommProductByPROId($_POST['soid']);

	while ($datass = mysqli_fetch_array($getSQProducts)) {
		//	$obj_so->setSQId($_POST['soid']);
		$getSQProduct = $obj_advancecommornon->getEstimateNonCommProductById($datass['PK_ESP_ID']);
...
if (isset($_POST['mode']) && $_POST['mode'] == 'getestAmount') {
	...
	if ($_POST['type'] == 1) {
		$result = $obj_advancecommornon->check_advamount_comm($_POST['estid']);
	} elseif ($_POST['type'] == 2) {
		$result = $obj_advancecommornon->check_advamount_noncomm($_POST['estid']);
	}
...
/*Multiple Advance */
/*
if(isset($_POST['mode']) && $_POST['mode']=='addAdvance_Comm'){
	$txt_pi_date = str_replace('/', '-', $_POST['txt_pi_date']);
...
if(isset($_POST['mode']) && $_POST['mode']=='editAdvance_Comm'){
	$txt_pi_date = str_replace('/', '-', $_POST['txt_pi_date']);
