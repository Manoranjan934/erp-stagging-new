...
function getProductIcon($name)
{
	...
	if (preg_match('/book/', $text)) {
		return "../assets/pdf_img/book.png";
	} elseif (preg_match('/calendar/', $text)) {
		return "../assets/pdf_img/calendar.png";
	} elseif (preg_match('/suitcase/', $text)) {
		return "../assets/pdf_img/suitcase.png";
	} elseif (preg_match('/bag/', $text)) {
		return "../assets/pdf_img/bag.png";
	} elseif (preg_match('/box/', $text)) {
		return "../assets/pdf_img/box.png";
	} elseif (preg_match('/pad/', $text)) {
		return "../assets/pdf_img/mattress.png";
	} else {
		return "../assets/pdf_img/product.png"; // default
	}
}
	...
	while ($datass = mysqli_fetch_array($getSCProducts)) {
		...
		while ($data = mysqli_fetch_array($getSCProduct)) {
            ...
            */
			...
			if (strtolower($data['types_name']) == 'product') {
				$productIcons[] = getProductIcon($data['itemnames']);
			}
		}
	}
...
$discount_field = ($datas['discount_field'] == 1) ? "Transport"
	: (($datas['discount_field'] == 2) ? "Direct"
		: "");
$discount =  number_format($datas['discount_final'], 2);
...
foreach ($productIcons as $icon) {
	$icons .= "<img src='{$icon}' style='width:25px;height:25px;margin-left:5px; margin-top:65px'>";
}
...
$html .= "<table style='border-width:unset;border-style:unset;border-color:unset;margin-bottom:0;border-bottom:0;'>
			...
			<th style='border:0;vertical-align: baseline;width: 30%;font-size:15px;float:left'><small>" . $createddate . "</small><br>{$icons}</th>
...
$html .= "<table style='border-width:unset;border-style:unset;border-color:unset;margin-bottom:0;border-bottom:0;'>
	<tr>
		<td colspan='5' style='border:0;padding:10px;font-size:12px;font-weight:800;text-align:right'><b>{$discount_field}</b> : {$discount}</td>
	</tr>
</table></body>
</html>";
