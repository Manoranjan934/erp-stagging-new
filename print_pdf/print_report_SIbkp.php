...
if (isset($data_id) && ($data_id != '')) {
    ...
    $getSQId = $obj_saleestimate->getSalesInvoiceById($data_id);
    ...
    $getSQAddress = $obj_saleestimate->getSalesInvoiceAddressById($data_id);
    $sqaddress = mysqli_fetch_array($getSQAddress);

    $unitArr = array();
    $scproduct = array();
    $gradeArr = array();
    $getSCProducts = $obj_saleestimate->getSalesInvoiceProductByPROId($data_id);

    while ($datass = mysqli_fetch_array($getSCProducts)) {
		
        $unitArr[] = $datass['uom'];
        $obj_saleestimate->setpk_id($data_id);
        $getSCProduct = $obj_saleestimate->getSalesInvoiceProductByIdForPDF($datass['PK_SEP_ID']);
...
<tr class='headdetails' style='height:100px'><th ><b>Details</b></th><th ><b>Category</b></th><th ><b>	Quantity</b></th><th style='text-align:right'>	<b>Price</b>[<img src='../assets/images/RS.png' style='width: 13px;height: 8px;'  />]</th><th style='text-align:right'>	<b>Total</b>[<img src='../assets/images/RS.png' style='width: 13px;height: 8px;'  />]</th>
</tr>
					</thead>";
					$html .= "<tbody>";
$itemsTotal = 0;
for ($i = 0; $i < count($scproduct); $i++) {
    ...
    $html .= "	<tr>
						...
						<td   style='text-transform:uppercase;padding-right: 10px;padding-top: 2px;padding-bottom:2px'>" . $scproduct[$i]['types_name'] . " - " . htmlspecialchars_decode($scproduct[$i]['itemnames']) . " </td>
...
$mpdf->WriteHTML($html);
$mpdf->Output('AV_Sales_Invoice.pdf', 'I');
