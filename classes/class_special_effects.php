...
class Special_effects {
	...
	public function __construct($c_id,$c_name,$c_cost,$c_description, $c_visibility,$c_created_on,$c_updated_on) {
	...
	function getallSpecialeffects(){
		 $query = "SELECT * FROM ".SPECIAL_EFFECTS." WHERE visibility = '1' ORDER BY pk_se_id DESC";
		$result = mysqli_query($GLOBALS["___mysqli_ston"], $query);
		return $result;
	}
	function AddSpecialeffects(){
		$res = 0;
		$sql = "SELECT * FROM ".SPECIAL_EFFECTS." WHERE name ='".$this->name."' AND visibility = '1'";
		...
		if($counts > 0){
			...
			$query = "INSERT INTO ".SPECIAL_EFFECTS." SET name ='".$this->name."',cost ='".$this->cost."', visibility = '1', description='".$this->description."'";
			$result = mysqli_query($GLOBALS["___mysqli_ston"], $query);
			$res = 1;
		}	
		return $res;
	}
	function geteditSpecialeffects(){
		$query = "SELECT * FROM ".SPECIAL_EFFECTS." WHERE  pk_se_id='".$this->id."' AND visibility = '1'";
		$result = mysqli_query($GLOBALS["___mysqli_ston"], $query);
		return $result;
	}
	function EditSpecialeffects(){
		$res = 0;
		$sql2 = "SELECT * FROM ".SPECIAL_EFFECTS." WHERE name ='".$this->name."' AND pk_se_id != '".$this->id."' AND visibility='1'";
		...
		if($counts1 > 0){
			...
			$query = "UPDATE ".SPECIAL_EFFECTS." SET name ='".$this->name."',cost ='".$this->cost."', visibility = '1', description='".$this->description."' WHERE pk_se_id =".$this->id."";
			$result = mysqli_query($GLOBALS["___mysqli_ston"], $query);
			$res = 1;
		}		
		return $res;
	...
	function deleteSpecialeffects(){
		$res = 0;
		$sql = "UPDATE ".SPECIAL_EFFECTS." SET visibility='0' WHERE pk_se_id=".$this->id."";
		$rest = mysqli_query($GLOBALS["___mysqli_ston"], $sql);
		if($rest){
			$res = 1;
		}
