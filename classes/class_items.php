...
class items {
	...
	function getAllitems($typ,$its){
		...
		$query = "SELECT pk_items_id,item.fk_item_id,item.type,  (IF(item.type =1, 'Product', (SELECT ts.types_name FROM ".TYPES." ts Where ts.pk_types_id = item.item_type))) as itemtypesval,first_price,second_price FROM ".ITEMS." item  WHERE  item.visibility = '1' AND item.type=".$typ." AND item.item_type=".$its."";
